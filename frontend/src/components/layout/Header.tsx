...
export default function Header() {
  ...
  useEffect(() => {
    setSupportEmail(
      normalizeSupportEmail(settings?.contact?.supportEmail || fallbackSupportEmail) || fallbackSupportEmail
    );
    setSupportLine(
      normalizeSupportLine(settings?.contact?.supportLineId || fallbackSupportLine) || fallbackSupportLine
    );
    setAppName(String(settings?.branding?.appName || fallbackAppName).trim() || fallbackAppName);
    setBrandingLogoUrl(String(settings?.branding?.logoUrl || "").trim());
    setBrandingPrimaryColor(String(settings?.branding?.primaryColor || "#111827").trim() || "#111827");
  }, [settings, fallbackSupportEmail, fallbackSupportLine, fallbackAppName]);
  const handleLogout = () => {
    logout();
    navigate("/");
  };
  ...
  return (
    <nav className="bg-white border-b border-gray-300 flex-shrink-0 relative z-50">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-gray-200 relative">
              ...
              <>                <div
                  ...
                  onClick={() => setIsProfileMenuOpen(false)} />
                    ...
                    }                    {user?.provider === 'local' && <button
                      onClick={() => {
                        setIsProfileMenuOpen(false);
                        setIsVerifyEmailModalOpen(true);
                      }}
                      className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors font-medium mb-1">
                        <Mail className="h-4 w-4" />                        {t("auth.email_verify.title", "Manage Email")}                      </button>}                    <button
      ...
      <VerifyEmailModal isOpen={isVerifyEmailModalOpen} onClose={() => setIsVerifyEmailModalOpen(false)} />
    </nav>);
}