...
export const PreviewLabel: React.FC<PreviewLabelProps> = ({
  ...
  const labelToSanitize =
  rawLabel.includes("&lt;") && rawLabel.includes("&gt;") ?
  decodeHtml(rawLabel) :
  rawLabel;
  const labelContent = sanitize(labelToSanitize);
  let finalHtml = labelContent.trim();
  if (field.required) {
    const asteriskHtml = '<span class="text-red-500 select-none ml-1">*</span>';
    const match = finalHtml.match(/(<\/(p|h[1-6]|li|div)>)$/i);
    if (match) {
      finalHtml = finalHtml.substring(0, finalHtml.length - match[1].length) + asteriskHtml + match[1];
    } else {
      finalHtml += asteriskHtml;
    }
  }
      ...
      <div className="mb-3 mt-1 space-y-3 flex flex-col items-center">        {}        {field.imageUrl &&
        ...
        <span
          ...
          style={isPublic ? { color: "var(--text)", opacity: 0.6 } : {}}>
            <span className={isPublic ? "" : "text-gray-500"}>            {questionNumber}          </span>          <span
