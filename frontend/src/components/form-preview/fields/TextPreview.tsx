...
import type { Field, TextField } from "@/types";
...
export const TextPreview: React.FC<PreviewFieldProps> = ({
  field,
  register,
  errors,
  questionNumber,
  ...
  if (validationType) {
    if (validationType === "Email") {
      ...
      validationRules.pattern = {
        value: /^[a-zA-Z0-9\s]*$/,
        message: t(
          "public.validation.alphanumeric",
      ...
      <div
        ...
        <p className="mt-1 text-sm text-gray-500 font-normal">{subLabel}</p>
          ...
          <input
                    ...
                    for (let i = 0; i < maskPattern.length; i++) {
                      ...
                      maskChar === "*")
                      {
                        ...
                        while (rawPtr < rawValue.length) {
                          ...
                          if (maskChar === "#" && isNum) isValid = true;else
                          if (maskChar === "@" && isLetter)
                          isValid = true;else
                          if (maskChar === "*" && (isNum || isLetter))
                          isValid = true;
                          if (isValid) {
                            formatted += char;
                            rawPtr++;
            ...
            readOnly={readOnly}
            maxLength={
            validation.hasInputMask && validation.inputMask ?
            validation.inputMask.length :
            hasMaxLength ?
            maxLength :
            undefined
            }
          ...
          <input
            ...
            "border-gray-300 bg-white"} rounded-lg text-black text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black focus:border-black transition-all ${
