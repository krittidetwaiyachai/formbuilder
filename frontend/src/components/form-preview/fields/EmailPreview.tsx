...
import { Field, EmailField, EmailFieldOptions, EmailValidation } from '@/types';
...
export const EmailPreview: React.FC<PreviewFieldProps> = ({ field, register, errors, questionNumber, isPublic }) => {
  ...
  const validationRules: RegisterOptions = {
    required: field.required ? (t('public.validation.required_field', { label: stripHtml(field.label) }) as string) : false,
    pattern: {
      value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i,
      message: t('public.validation.invalid_email', "Invalid email address") as string
    }
  };
  ...
  if (hasMaxLength && maxLength) {
    validationRules.maxLength = {
      ...
      message: t('public.validation.max_length', { count: Number(maxLength) }) as string
    };
  }
    ...
    validationRules.validate = {
      ...
      noFreeEmail: (value: string) => {
        ...
        if (domain && FREE_EMAIL_DOMAINS.includes(domain)) {
          return t('public.validation.business_email', "Please use a business/work email address") as string;
        }
        return true;
      }
    };
        ...
        <div className="relative group" title={hoverText}>
          {!isPublic && (
            ...
            <input
                ...
                readOnly={readOnly}
                maxLength={hasMaxLength ? (maxLength as unknown as number) : undefined}
          ...
          <div className="mt-4">
            ...
            <input
                ...
                {...register(`${fieldName}_confirm`, {
                    required: field.required ? (t('public.validation.required_field', { label: t('public.validation.confirm_email_label') }) as string) : false,
                    pattern: {
                        value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i,
                    ...
                    maxLength: field.validation?.maxLength ? {
                        ...
                        message: t('public.validation.max_length', { count: Number(field.validation.maxLength) }) as string
                    } : undefined,
                    minLength: field.validation?.minLength ? {
                        value: field.validation.minLength as unknown as number,
                        message: t('public.validation.min_length', { count: Number(field.validation.minLength) }) as string
                    } : undefined,
                ...
                placeholder={field.placeholder ? (field.placeholder as string) : t('public.validation.confirm_email_placeholder', "Confirm your email")}
                defaultValue={defaultValue as string}
