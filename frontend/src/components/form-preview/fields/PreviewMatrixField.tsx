...
export const PreviewMatrixField: React.FC<PreviewMatrixFieldProps> = ({
  ...
  const currentValue = (watch(fieldName) || {}) as Record<
    string,
    string | string[]>;
  ...
  return (
    <div
      className={`mb-6 w-full ${hidden ? "hidden" : ""} ${isRowLayout ? "flex items-start gap-6" : ""}`}
      title={hoverText}>
      {}      <div
        className={`${isRowLayout ? "w-48 flex-shrink-0 pt-2" : "mb-3"} ${labelAlignment === "RIGHT" ? "text-right" : ""}`}>
        <PreviewLabel
          field={field}
        ...
        <p className="mt-1 text-sm text-gray-500 font-normal">{subLabel}</p>
          ...
          className="w-full rounded-xl border shadow-sm relative group overflow-hidden"
          style={{
          ...
          {}          <div className="md:hidden absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-black/5 to-transparent pointer-events-none z-10 rounded-r-xl" />          <div className="overflow-x-auto pb-1">            <table className="min-w-full divide-y divide-[color:var(--divider)]">              <thead>                <tr style={{ backgroundColor: "var(--input-bg)" }}>                  <th
                    scope="col"
                    className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider min-w-[150px] sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]"
                    style={{
                      color: "var(--text)",
                      opacity: 0.7,
                ...
                <tr
                  ...
                  style={{
                    ...
                    <td
                    className="px-4 py-3.5 whitespace-normal text-sm font-medium sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]"
                    style={{
                      color: "var(--text)",
                      backgroundColor:
                      rowIndex % 2 === 0 ?
                  ...
                  <td
                    ...
                    className="px-4 py-3.5 whitespace-nowrap text-center">
                        ...
                        style={{
                          borderColor: isChecked(row.id, col.label) ?
                          "var(--primary)" :
                          "var(--input-border)",
                          backgroundColor: isChecked(row.id, col.label) ?
                          "var(--primary)" :
                          "transparent"
                        }}
                        className={`
            ...
            validate: (value) => {
              ...
              if (inputType === "radio") {
                if (value && typeof value === "object") {
                  ...
                  const answeredRows = Object.keys(typedValue).filter(
                    (k) => typedValue[k]
                  ).length;
                  if (answeredRows !== rows.length) {
                    return t(
                      "public.validation.matrix_radio",
              ...
              if (inputType === "checkbox") {
                const hasAnswers = Object.values(value || {}).some(
                  (arr: unknown) => Array.isArray(arr) && arr.length > 0
                );
                return (
                  hasAnswers ||
                  t(
                    "public.validation.matrix_checkbox",
