...
import { Field, RateField, RateFieldOptions } from '@/types';
...
export const PreviewRateField: React.FC<PreviewFieldProps> = ({ field, register, errors, watch, setValue, questionNumber, isPublic }) => {
  ...
  const getIcon = (idx: number) => {
    ...
    const IconComponent = ({
      ...
      'thumb': ThumbsUp,
      'smile': Smile
    } as Record<string, any>)[normalizedIcon] || Star;

    let colorClass = 'text-gray-300';
    if (isFilled) {
       switch(normalizedIcon) {
           case 'heart': colorClass = 'fill-current text-red-400'; break;
           case 'shield': colorClass = 'fill-current text-blue-400'; break;
           case 'zap': colorClass = 'fill-current text-amber-400'; break;
           case 'flag': colorClass = 'fill-current text-green-400'; break;
           case 'thumbsup': 
           case 'thumb': colorClass = 'fill-current text-blue-500'; break;
           case 'smile': colorClass = 'fill-current text-yellow-500'; break;
           default: colorClass = 'fill-current text-yellow-400';
       }
    }
          ...
          <div className={`flex items-center gap-1 ${labelAlignment === 'CENTER' ? 'justify-center' : ''}`}>
            {Array.from({ length: maxRating }).map((_, index) => {
              ...
              <button
                ...
                onClick={() => {
                  if (!readOnly) {
                     setValue(fieldName, starIdx.toString(), { shouldValidate: true, shouldDirty: true });
                     setRating(starIdx);
                     setHoverRating(null);
                  }
                }}
            ...
            {...register(fieldName, {
              ...
              validate: (value) => {
                ...
                if (field.required && (!value || isNaN(numVal) || numVal < 1)) {
                  return t('public.validation.rate_min', "Please select at least 1"); // Adjusted message key might be needed
                }
                return true;
              }
            })}
