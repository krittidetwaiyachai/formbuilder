...
import type { Field, AddressField } from "@/types";
...
export const PreviewAddressField: React.FC<PreviewFieldProps> = ({
  ...
  const getBaseInputClass = (hasIcon: boolean = false) => {
    if (isPublic) {
      return `w-full px-4 ${
      shrink ? "py-2 text-base" : "py-3 text-base"} border placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[color:var(--primary)]/20 focus:border-[color:var(--primary)] transition-all hover:border-gray-300`;
    }
      ...
      <div
        ...
        <PreviewLabel
          ...
          <AddressInput
            ...
            label={
            sublabels.street2 ||
            t("public.address.street2", "Street Address Line 2")
            }
            placeholder={placeholders.street2}
            defaultLabel={t(
              "public.address.street2",
            ...
            className={`grid ${
            showCountry ? "grid-cols-2" : "grid-cols-1"} gap-3`
            }>
            {showZip &&
            <AddressInput
              id={`${fieldName}_zip`}
              name={`${fieldName}_zip`}
              label={
              sublabels.zip || t("public.address.zip", "Postal / Zip Code")
              }
            ...
            <AddressInput
              ...
              label={
              sublabels.country || t("public.address.country", "Country")
              }
