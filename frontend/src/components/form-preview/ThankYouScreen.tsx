...
import type { FormSettings, ThankYouScreenSettings } from "@/types";
import QuizResults from "./QuizResults";
import type { QuizReview } from "./QuizResults";
...
export default function ThankYouScreen({
  ...
  const message =
    globalSettings?.successMessage || settings?.message || defaultMessage;
  ...
  useEffect(() => {
    if (settings?.showConfetti) {
      ...
      confettiContainer.innerHTML = Array.from({ length: 50 })
        .map(() => {
