    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">        <motion.div
          ...
          <div
            ...
            {t("public.already_submitted_title")}          </h2>          <p className="mb-6" style={{ color: "var(--text)", opacity: 0.8 }}>            {t("public.already_submitted_desc")}          </p>        </motion.div>      </div>);
  }
  if (!form) {
    return (
      ...
      return (
        <div
          ...
          <div className="relative z-10 w-full flex justify-center">            <ThankYouScreen
              ...
              showExplanation={
              !!(form.isQuiz && form.quizSettings?.showExplanation)
              }
  ...
  return (
    <PublicFormLayout
      ...
      <AnimatePresence mode="wait">        <motion.div
          ...
          className={
          isCardLayout ?
          `relative flex flex-col ${isMobileView ? "max-h-[90vh] mx-2" : isTabletView ? "max-h-[85vh] mx-4" : "max-h-[80vh] mx-auto w-full max-w-3xl"} border shadow-2xl rounded-2xl overflow-hidden backdrop-blur-xl transition-all duration-300` :
          "space-y-4"
          }
          style={{
            backgroundColor: "var(--card-bg)",
          ...
          <div
            className={`${isCardLayout ? "relative z-20 flex-shrink-0 px-8 pt-8 pb-4 border-b" : "rounded-xl shadow-sm border p-6 md:p-8 mb-6 backdrop-blur-md"}`}
            style={{
            ...
            <div
              ...
              <div className="flex justify-center mb-6">                    <img
                ...
                className={`font-bold ${isMobileView ? "text-lg" : isTabletView ? "text-xl" : isCardLayout ? "text-2xl md:text-3xl" : "text-3xl"}`}
                style={{ color: "var(--text)" }}>
                  {form.title}                </h1>                {form.description &&
              <div
                className="prose prose-sm max-w-none mt-2"
                dangerouslySetInnerHTML={{
                  __html: sanitize(form.description)
                }}
            ...
            className={`${isCardLayout ? `flex-1 overflow-y-auto ${isMobileView ? "p-4" : isTabletView ? "p-5" : "p-6"}` : `flex flex-col rounded-xl shadow-sm border backdrop-blur-md ${isMobileView ? "p-4" : "p-6 md:p-8"}`}`}
            style={
            !isCardLayout ?
            {
              backgroundColor: "var(--card-bg)",
            ...
            !(
            isCardLayout &&
            currentCardIndex < currentPageFields.length - 1 ||
