...
export function useFormNavigation({
  ...
  const validateCurrentPage = useCallback(async () => {
    ...
    for (const f of requiredFields) {
      const fieldName = `field_${f.id}`;
      if (f.type === FieldType.FULLNAME) {
        if (isEmpty(getValues(`${fieldName}_first`))) manualValid = false;else
        if (isEmpty(getValues(`${fieldName}_last`))) manualValid = false;
      } else if (f.type === FieldType.ADDRESS) {
        const opts = (f.options || {}) as AddressFieldOptions;
        const vStreet = getValues(`${fieldName}_street`);
        const vCity = getValues(`${fieldName}_city`);
        if (opts.showStreet !== false && isEmpty(vStreet)) manualValid = false;else
        if (opts.showCity !== false && isEmpty(vCity)) manualValid = false;
      } else {
