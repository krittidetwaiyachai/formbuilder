...
export default function DashboardFormCard({
  ...
  const allEditors = (
  [form.createdBy, ...(form.collaborators || [])] as (UserInfo | undefined)[]).
  ...
  return (
    <div
      ...
      {}      <div
        className={`${compact ? "h-16" : "h-24"} bg-gray-50/50 border-b border-gray-100 relative overflow-hidden group-hover:bg-gray-50 transition-colors`}>
        <div className="absolute top-3 left-3 z-10">          <div className="flex items-center gap-2">            <span
          ...
          className={`absolute ${compact ? "-bottom-6 -right-6 text-gray-100" : "-bottom-4 -right-4 text-gray-100"} transform rotate-12 group-hover:scale-105 group-hover:rotate-6 transition-all duration-500`}>
          <FileText
            className={`${compact ? "w-20 h-20" : "w-24 h-24"} opacity-50`} />
        </div>      </div>      <div className={`${compact ? "p-3" : "p-5"} flex-1 flex flex-col`}>        <div className="mb-3">          <h3
            className={`${compact ? "text-base" : "text-lg"} font-bold text-gray-900 mb-1.5 line-clamp-2 group-hover:text-black transition-colors leading-snug tracking-tight`}>
            ...
            <div
              className={`flex -space-x-2 overflow-hidden pointer-events-none items-center ${compact ? "scale-90 origin-left" : ""}`}>
