    ...
    () => [
      ...
      columnHelper.accessor("lastActiveAt", {
        ...
        cell: (info) => {
          ...
          if (!value) {
            return <span className="text-gray-400">{t("admin.users.status.unknown")}</span>;
          }
          return (
              ...
              {isEditing ? (
                <div className="flex items-center gap-2">
                  <Select
                    ...
                    <SelectTrigger className="h-8 w-[170px] rounded-lg border-blue-200 bg-blue-50 text-blue-900 text-xs font-semibold shadow-none focus:ring-2 focus:ring-blue-500/20 focus:ring-offset-0">
                      <SelectValue placeholder={t("admin.users.action.select_role")} />
                    </SelectTrigger>
                    <SelectContent className="rounded-xl border-blue-100 p-1 shadow-xl">
                      {roles.map((role) => (
                        <SelectItem
                          key={role.id}
                          value={role.id}
                          className="rounded-lg text-xs font-semibold tracking-wide data-[state=checked]:bg-blue-600 data-[state=checked]:text-white">
                          {role.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <button
                    ...
                    title={t("admin.users.action.cancel_role")}>
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
        <p className="text-sm text-gray-500">{t("admin.users.page_info", { page, total: totalPages })}</p>
        <div className="flex items-center gap-2">
          <button
