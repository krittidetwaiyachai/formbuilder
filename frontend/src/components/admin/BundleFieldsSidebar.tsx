...
export const fieldCategories: { name: string; fields: SidebarFieldConfig[] }[] =
  [
    ...
    {
      name: "Rating",
      fields: [{ type: FieldType.RATE, label: "Star Rating", icon: Star }],
    },
    {
      name: "Layout",
      fields: [
...
function FieldTypeButton({
  ...
  const translatedLabel =
    labels[config.label as keyof ReturnType<typeof useFieldLabels>] ||
    config.label;
...
export default function BundleFieldsSidebar() {
  ...
  return (
    <div
      className={`bg-white border-r border-gray-200 flex flex-col h-full shadow-sm z-20 transition-all duration-300 ease-in-out ${isCollapsed ? "w-16" : "w-[300px]"} relative`}
    >
      <button
      ...
      <Droppable
        ...
        renderClone={(provided, _snapshot, rubric) => {
          const fieldType = rubric.draggableId.replace(
            "sidebar-",
            "",
          ) as FieldType;
