...
export default function CreateLocalUserModal({
  ...
  const passwordStrength = useMemo(() => {
    ...
    if (!value) {
      return { score: 0, level: 1, label: t("admin.users.password_strength.very_low") };
    }
    let score = 0;
    ...
    let level = 1;
    let label = t("admin.users.password_strength.very_low");
    if (score <= 2) {level = 1;label = t("admin.users.password_strength.very_low");} else
    if (score <= 4) {level = 2;label = t("admin.users.password_strength.low");} else
    if (score <= 5) {level = 3;label = t("admin.users.password_strength.medium");} else
    if (score <= 6) {level = 4;label = t("admin.users.password_strength.high");} else
    {level = 5;label = t("admin.users.password_strength.very_high");}
                  ...
                  <FormField label={t("admin.users.create_password")} required>
                    ...
                    <div className="mt-2 space-y-1.5">
                      ...
                      <div
                        ...
                        passwordStrength.level === 3 ? "bg-yellow-400" :
                        passwordStrength.level === 4 ? "bg-emerald-400" :
                        "bg-emerald-600" :
                        "bg-gray-200"}`
                        } />
                      )}
...
function SuccessView({
  ...
  showPassword,
  onToggleShow
}: {user: AdminCreateLocalUserResponse;createdPassword: string;onCopy: () => void;copied: boolean;showPassword: boolean;onToggleShow: () => void;}) {
  const { t } = useTranslation();
  return (
