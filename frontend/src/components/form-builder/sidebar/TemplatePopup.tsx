...
import { Plus, Layers, MapPin, Briefcase, CalendarCheck, MessageSquare, Share2, GraduationCap, Star, Lock, User } from 'lucide-react';
...
export const TemplatePopup = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleAddBundle = (bundle: Bundle) => {
    const mappedBundle = {
        title: bundle.name,
        fields: bundle.fields as Array<Pick<Field, 'type' | 'label'> & Partial<Field>>
    };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-150">
          ...
          <div className="p-6 overflow-y-auto flex-1">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {bundles.map((bundle) => {
                    const IconComponent = bundle.options?.icon && IconMap[bundle.options.icon] ? IconMap[bundle.options.icon] : Layers;
