...
export default function MobileDrawer({ 
    ...
    const { activeSidebarTab, setActiveSidebarTab, undo, redo, historyIndex, history } = useFormStore();
        ...
        <>
            {drawerContent && (
                <div className="md:hidden fixed inset-0 z-[9999] flex">
                    ...
                    <div className={`relative w-full max-w-[90%] md:max-w-[400px] h-full bg-white shadow-2xl animate-in ${drawerContent === 'fields' ? 'slide-in-from-left mr-auto md:rounded-r-2xl' : 'slide-in-from-right ml-auto md:rounded-l-2xl'} duration-300 flex flex-col overflow-hidden`}>
            ...
            <div className="md:hidden absolute bottom-40 right-10 z-[90]">
                 <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    >
                        {isOpen && <span className="speed-dial-label" style={{ marginTop: '-50px' }}>{t('builder.settings')}</span>}
                        ...
                        {activeSidebarTab === 'logic' ? (
                            <button
                                onClick={() => { setActiveSidebarTab('builder'); setIsOpen(false); }}
                            ...
                            >
                                {isOpen && <span className="speed-dial-label" style={{ marginTop: '-14px' }}>{t('builder.back_to_canvas')}</span>}
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
                            </button>
                        ) : (
                            <button
                                onClick={() => { setActiveSidebarTab('logic'); setIsOpen(false); }}
                                className={`absolute w-10 h-10 bg-black text-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ease-out delay-75 ${
                                    isOpen 
                                    ? '-translate-y-[75px] -translate-x-[45px] opacity-100 scale-100' 
                                    : 'translate-y-0 translate-x-0 opacity-0 scale-50 pointer-events-none'
                            ...
                            >
                                {isOpen && <span className="speed-dial-label" style={{ marginTop: '-14px' }}>{t('builder.tabs.logic')}</span>}
                    ...
                    >
                        {isOpen && <span className="speed-dial-label" style={{ marginTop: '0px' }}>{t('builder.properties.title')}</span>}
                    ...
                    >
                        {isOpen && <span className="speed-dial-label">{t('builder.add_field')}</span>}
                    ...
                    >
                        {isOpen && <span className="speed-dial-label">{t('builder_header.undo')}</span>}
                    ...
                    >
                        {isOpen && <span className="speed-dial-label" style={{ marginTop: '25px' }}>{t('builder_header.redo')}</span>}
