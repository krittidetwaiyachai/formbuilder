...
import type { Field, Column, TableField as TableFieldType } from "@/types";
...
export const TableField: React.FC<TableFieldProps> = ({
  ...
  const columns = tableField.options?.columns || [
  { id: "c1", label: `${t("builder.field.column")} 1` },
  { id: "c2", label: `${t("builder.field.column")} 2` }];
  const allowAddRow =
  tableField.options?.allowAddRow !== undefined ?
  tableField.options.allowAddRow :
  true;
  const updateColumns = (newCols: Column[]) => {
    if (updateField) {
      updateField(field.id, {
        options: { ...field.options, columns: newCols }
      });
    }
  };
            ...
            <th
              ...
              className="px-4 py-3 border border-gray-200 font-semibold min-w-[150px] relative group/col">
                ...
                <input
                  ...
                  value={col.label}
                  onChange={(e) =>
                  handleColumnLabelChange(idx, e.target.value)
                  }
