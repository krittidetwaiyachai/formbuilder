...
export const EmailField: React.FC<EmailFieldProps> = ({ field, fieldStyle, disabledClass = "opacity-60 cursor-pointer" }) => {
  ...
  const sublabel = (field.validation as any)?.sublabel;
    ...
    <div className="space-y-3 pointer-events-none">
       <div className="relative max-w-full group">
         ...
         <input
           ...
           readOnly
           tabIndex={-1}
           className={`w-full pl-12 pr-4 py-3.5 border ${fieldStyle.inputBorder} rounded-xl bg-blue-50/30 text-black text-base shadow-sm transition-all duration-300 ${disabledClass} group-hover:bg-white group-hover:shadow-md ${isReadOnly ? 'opacity-50' : ''}`}
         />
       </div>
