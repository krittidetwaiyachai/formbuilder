...
export const TimeProperties: React.FC<TimePropertiesProps> = ({ field, updateField, duplicatesField }) => {
  const [activeTab, setActiveTab] = useState<'general' | 'options' | 'range' | 'advanced'>('general');
  ...
  return (
    <div className="space-y-4">
      {/* Tabs */}
      <div className="flex items-center gap-1 mb-3 bg-gray-100 p-1 rounded-md overflow-x-auto">
        {['general', 'options', 'range', 'advanced'].map((tab) => (
          <button
            key={tab}
