...
import type { Field, TimeFieldOptions } from "@/types";
...
export const TimeField: React.FC<TimeFieldProps> = ({
  ...
  <div className="flex items-center gap-2">      <div>        <input
        ...
        <label className="block text-xs text-gray-400 mt-1 text-center">          {subLabelHour}        </label>      </div>      <span className="text-gray-400 font-bold mb-6">:</span>      <div>        <input
        type="text"
    ...
    <div className="mb-6">          <select
        disabled
        className={`h-[46px] px-2 border ${fieldStyle.inputBorder} rounded-lg bg-gray-50/50 text-sm shadow-sm ${disabledClass} pointer-events-none`}>
            <option>AM</option>            <option>PM</option>          </select>        </div>
