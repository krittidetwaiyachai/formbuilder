...
export default function QuizModeBanner({
  form,
  onOpenSettings,
}: QuizModeBannerProps) {
  ...
  const usedScore =
    form.fields?.reduce((sum, f) => sum + (f.score || 0), 0) || 0;
  const difference = totalScore - usedScore;

  const status =
    difference === 0 ? "complete" : difference > 0 ? "incomplete" : "exceeded";
  ...
  const statusConfig = {
    ...
    exceeded: {
      ...
      messageColor: "text-gray-200",
      message: t("builder.quiz.exceeded_points", {
  ...
  return (
    <motion.div
      ...
      className={`sticky top-0 z-30 mb-4 px-4 py-3 rounded-xl bg-black text-white shadow-xl border max-w-[700px] mx-auto ${config.border}`}
    >
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-3 border-t md:border-t-0 border-white/10 pt-2 md:pt-0 mt-1 md:mt-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-semibold ${config.messageColor}`}>
              {config.message}
            </span>
          </div>
