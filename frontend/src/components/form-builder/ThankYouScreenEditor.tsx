...
export default function ThankYouScreenEditor({
  currentForm,
  updateForm
}: ThankYouScreenEditorProps) {
  ...
  <div className="flex flex-col items-center justify-center w-full text-center px-4">    {}    <div className="relative group w-full mb-6">      <input
        ...
        layout === "cover" ?
        "text-white placeholder-white/50 drop-shadow-md" :
        "text-gray-900"}`
        }
        ...
        <div
        className={`absolute -inset-2 border-2 border-transparent group-hover:border-dashed group-hover:border-gray-200 rounded-lg pointer-events-none transition-colors ${
        ...
        layout === "cover" ?
        "text-white/80 placeholder-white/40 drop-shadow" :
        "text-gray-500"}`
        }
  ...
  <div className="relative mb-8">    {}    <div
      ...
      iconColor === "white" ?
      "bg-gray-200" :
      iconStyles[iconColor]?.split(" ")[0]?.replace("bg-", "bg-") ||
      "bg-emerald-200"}`
      } />
      <div
      className={`w-28 h-28 rounded-[2rem] flex items-center justify-center relative z-10 shadow-xl border-4 border-white ${iconStyles[iconColor] || iconStyles.green}`}>
  ...
  <div className="absolute bottom-6 left-0 right-0 flex justify-center w-full px-8">    <div className="relative group max-w-xs w-full">      <input
        ...
        "text-gray-400 focus:text-gray-600"}`
        }
        ...
        <div
        className={`absolute -inset-2 border border-transparent group-hover:border-dashed group-hover:border-gray-200 rounded pointer-events-none transition-colors ${
  ...
  return (
    <div className="flex items-center justify-center min-h-full p-8 animate-in fade-in duration-500 w-full h-full bg-gray-50/50">      <div
        className={`w-full ${layout === "simple" ? "max-w-4xl" : "max-w-6xl"} bg-white shadow-2xl shadow-gray-200/50 rounded-[2.5rem] border border-white overflow-hidden flex ${
        layout === "split-left" ?
        "flex-row" :
        layout === "split-right" ?
        "flex-row-reverse" :
        "flex-col"} relative min-h-[600px]`
        }>
        {}        {layout === "simple" &&
