...
export function FieldContextMenu({
  ...
  const setActiveSidebarTab = useFormStore(
  ...
  const additionalSelectedIds = useFormStore(
    (state) => state.additionalSelectedIds
  );
  const isMultiSelect =
  ...
  const handleGroupFields = () => {
    ...
    const idsToGroup = [selectedFieldId, ...additionalSelectedIds].filter(
      Boolean
    ) as string[];
  ...
  []),
  {
    ...
    t("builder.context.optional") :
    t("builder.context.require"),
  ...
  },
  {
    ...
    label: field.options?.hidden ?
    t("builder.context.unhide") :
    t("builder.context.hide"),
    onClick: () => {
  ...
  { type: "divider" },
  {
    ...
    onClick: () => {
      if (!currentForm?.fields) return;
      const currentIndex = currentForm.fields.findIndex(
        (f) => f.id === field.id
      );
      if (currentIndex > 0) {
