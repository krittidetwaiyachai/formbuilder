...
export default function WelcomeScreenEditor({
  ...
  <div className="max-w-2xl w-full text-center space-y-8 p-12">      {}      <div className="flex justify-center mb-8">        <div
        ...
        updateForm({
          welcomeSettings: {
            ...
            showStartButton:
            currentForm?.welcomeSettings?.showStartButton ?? true
          }
        })
        }
        ...
        layout === "cover" ?
        "text-white/90 placeholder-white/50 drop-shadow" :
        "text-gray-500"}`
        }
    ...
    <div className="flex justify-center pt-4">          <div className="relative group">            <input
          ...
          value={
          currentForm?.welcomeSettings?.buttonText ||
          t("builder.welcome.start_button")
          }
          onChange={(e) =>
          updateForm({
            welcomeSettings: {
              ...
              title: currentForm?.welcomeSettings?.title || "",
              description:
              currentForm?.welcomeSettings?.description || "",
              showStartButton:
              currentForm?.welcomeSettings?.showStartButton ?? true,
  ...
  return (
    <div className="flex items-center justify-center min-h-full p-8 animate-in fade-in duration-500 w-full h-full bg-gray-50/50">      <div
        className={`w-full ${layout === "simple" ? "max-w-6xl" : "max-w-7xl"} bg-white shadow-2xl shadow-gray-200/50 rounded-[2.5rem] overflow-hidden flex ${
        layout === "split-left" ?
        "flex-row" :
        layout === "split-right" ?
        "flex-row-reverse" :
        "flex-col"} relative min-h-[600px] border border-white`
        }>
        {}        {layout === "simple" &&
