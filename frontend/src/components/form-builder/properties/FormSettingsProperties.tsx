...
export function FormSettingsProperties({
  ...
  const [hasResponseLimit, setHasResponseLimit] = useState(
    !!settings.responseLimit
  );
  const [activeTab, setActiveTab] = useState<
    "general" | "submission" | "display">(
    "general");
  ...
  const { shouldScrollToQuizSettings, setShouldScrollToQuizSettings } =
  useFormStore();
  ...
  currentForm.isQuiz,
  shouldScrollToQuizSettings,
  setShouldScrollToQuizSettings]
  );
  return (
    <>      {}      <div className="flex items-center gap-1 mb-4 bg-gray-100 p-1 rounded-md">        {["general", "submission", "display"].map((tab) =>
        ...
        <div className="space-y-4">          <div>            <label className="block text-sm font-medium text-black mb-1">              {t("settings.general.title")}            </label>            <input
            ...
            "border-gray-200 bg-white hover:border-gray-300"}
              ...
              <div className="flex items-center gap-4">              <div
                ...
                `}>
                  ...
                  className={`font-semibold text-base ${currentForm.isQuiz ? "text-black" : "text-gray-700"}`}>
                    {t("settings.general.quiz_mode")}                </h4>                <p className="text-xs text-gray-500 mt-1 font-medium">                  {t("settings.general.quiz_desc")}                </p>              </div>            </div>            {}            <div
              className={`
          ...
          <div
            ...
            className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wide">              {t("settings.quiz.settings")}            </h4>            {}            {}            <div className="pt-3 border-t border-gray-200">              <p className="text-xs font-semibold text-gray-700 mb-3 uppercase">                {t("settings.quiz.respondents_see")}              </p>              <div className="space-y-3">                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.total_score")}                  </label>                  <input
                    type="number"
                    min="1"
                    value={currentForm.quizSettings?.totalScore || 100}
                    onChange={(e) =>
                    handleFormUpdate("quizSettings", {
                      ...currentForm.quizSettings,
                      totalScore: parseInt(e.target.value) || 100
                    })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-transparent" />
                      <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.total_score_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.pass_percentage")}                  </label>                  <input
                    type="number"
                    min="1"
                    max="100"
                    value={currentForm.quizSettings?.passPercentage ?? 50}
                    onChange={(e) =>
                    handleFormUpdate("quizSettings", {
                      ...currentForm.quizSettings,
                      passPercentage: e.target.value ? parseInt(e.target.value) : undefined
                    })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-transparent" />                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.pass_percentage_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.show_score")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.show_correct_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.missed_questions")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      checked={
                      currentForm.quizSettings?.
                      allowViewMissedQuestions || false
                      }
                      onChange={(e) =>
                      handleFormUpdate("quizSettings", {
                        ...currentForm.quizSettings,
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.missed_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.point_values")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      checked={
                      currentForm.quizSettings?.showDetail || false
                      }
                      onChange={(e) =>
                      handleFormUpdate("quizSettings", {
                        ...currentForm.quizSettings,
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.point_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.show_explanation")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      checked={
                      currentForm.quizSettings?.showExplanation || false
                      }
                      onChange={(e) =>
                      handleFormUpdate("quizSettings", {
                        ...currentForm.quizSettings,
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.explanation_desc")}                  </p>                </div>              </div>            </div>            {}            <div className="pt-3 border-t border-gray-200">              <p className="text-xs font-semibold text-gray-700 mb-3 uppercase">                {t("settings.quiz.options")}              </p>              <div className="space-y-3">                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.shuffle")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      checked={
                      currentForm.quizSettings?.shuffleQuestions || false
                      }
                      onChange={(e) =>
                      handleFormUpdate("quizSettings", {
                        ...currentForm.quizSettings,
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.shuffle_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.require_signin")}                  </label>                  <label className="relative inline-flex items-center cursor-pointer">                    <input
                      type="checkbox"
                      checked={
                      currentForm.quizSettings?.requireSignIn || false
                      }
                      onChange={(e) =>
                      handleFormUpdate("quizSettings", {
                        ...currentForm.quizSettings,
                      ...
                      className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-black"></div>                  </label>                  <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.signin_desc")}                  </p>                </div>              </div>            </div>            {}            <div className="pt-3 border-t border-gray-200">              <p className="text-xs font-semibold text-gray-700 mb-3 uppercase">                {t("settings.quiz.timing")}              </p>              <div className="space-y-3">                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.time_limit")}                  </label>                  <input
                    type="number"
                    min="1"
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black focus:border-transparent" />
                      <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.time_limit_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.start_time")}                  </label>                  <input
                    type="datetime-local"
                    lang="en-GB"
                    step="60"
                    value={
                    currentForm.quizSettings?.startTime ?
                    ...
                    style={{ fontVariantNumeric: "tabular-nums" }} />
                      <p className="mt-1 text-xs text-gray-500">                    {t("settings.quiz.start_time_desc")}                  </p>                </div>                {}                <div>                  <label className="block text-sm font-medium text-black mb-1">                    {t("settings.quiz.end_time")}                  </label>                  <input
                    type="datetime-local"
                    lang="en-GB"
                    step="60"
                    value={
                    currentForm.quizSettings?.endTime ?
                    toLocalDateTimeString(
                      currentForm.quizSettings.endTime
                    ) :
                    ""
        ...
        <div className="space-y-4">          <div>            <label className="block text-sm font-medium text-black mb-1">              {t("settings.submission.submit_text")}            </label>            <input
              ...
              <p className="mt-1 text-xs text-gray-500">              {t("settings.submission.success_desc")}            </p>          </div>          <div>            <label className="block text-sm font-medium text-black mb-1">              {t("settings.submission.allow_multiple")}            </label>            <label className="relative inline-flex items-center cursor-pointer">              <input
                ...
                onChange={(e) =>
                updateSettings("allowMultipleSubmissions", e.target.checked)
                }
                className="sr-only peer" />
            ...
            <div className="mt-2">              <input
                ...
                className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text"
                placeholder={t(
                  "builder.settings.max_responses_placeholder"
                )} />
                </div>
        ...
        <div className="space-y-4">          {}          <div>            <label className="block text-sm font-medium text-black mb-2">              {t("settings.display.layout")}            </label>            <div className="grid grid-cols-2 gap-3">              {}              <button
                ...
                onChange={(e) =>
                updateSettings("showQuestionNumber", e.target.checked)
                }
                className="sr-only peer" />
