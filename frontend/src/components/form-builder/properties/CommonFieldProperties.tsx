...
import type { Field, Form, TypedField, CheckboxOption } from "@/types";
...
export function CommonFieldProperties({
  field,
  currentForm,
  updateField
}: CommonFieldPropertiesProps) {
    ...
    <>      {!hasCustomRendering &&
      ...
      <>          <div className="flex items-center">            <input
        ...
        <div className="space-y-4 p-4 bg-white border-2 border-indigo-200 rounded-xl mt-4 shadow-sm">              <div className="flex items-center justify-between mb-1">                <h4 className="font-semibold text-indigo-900 flex items-center gap-2 text-sm">                  {t("builder.properties.quiz_settings")}                </h4>              </div>              {}              <div>                <label className="block text-sm font-medium text-gray-700 mb-2">                  {t("builder.properties.score")}                </label>                {(() => {
              const totalScore =
              currentForm.quizSettings?.totalScore || 100;
              const usedScore =
              currentForm.fields?.
            ...
            hasArrayOptions(field as TypedField) &&
            (field as TypedField).options?.items &&
            (((field as TypedField).options?.items as CheckboxOption[])?.
            length ?? 0) > 0 ?
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white">
                    <option value="">                      {t("builder.properties.select_correct_option")}                    </option>                    {(hasArrayOptions(field as TypedField) ?
              (field as TypedField).options?.
              items as CheckboxOption[] || [] :
              []).
              map((opt: CheckboxOption, idx: number) =>
              <option key={idx} value={opt.value}>                        {opt.label}                      </option>
              )}                  </select> :
            field.type === FieldType.CHECKBOX &&
            hasCheckboxOptions(field as TypedField) &&
            (field as TypedField).options?.items &&
            (((field as TypedField).options?.items as CheckboxOption[])?.
            length ?? 0) > 0 ?
            <div className="space-y-2 p-3 border border-gray-300 rounded-lg bg-white max-h-48 overflow-y-auto">                    <p className="text-xs text-gray-500 mb-2">                      {t("builder.properties.select_all_correct")}                    </p>                    {(
              (field as TypedField).options?.
              items as CheckboxOption[] || []).
              map((opt: CheckboxOption, idx: number) => {
                ...
                return (
                  <label
                    ...
                    className="flex items-center gap-2 cursor-pointer hover:bg-indigo-50 p-1.5 rounded transition-colors">
                      ...
                      checked={isChecked}
                      onChange={(e) => {
                        let newAnswers = correctAnswers.filter(
                          (a) => a !== opt.value
                        );
                        if (e.target.checked) {
