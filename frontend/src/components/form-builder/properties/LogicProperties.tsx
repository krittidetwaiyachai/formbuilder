...
import { hasArrayOptions, hasCheckboxOptions } from "@/types";
...
export const LogicProperties: React.FC<LogicPropertiesProps> = ({ field }) => {
  ...
  const { currentForm, addCondition, updateCondition, deleteCondition } =
  useFormStore();
  const conditions = currentForm?.conditions || [];
  const fieldConditions = conditions.filter(
    (c) => c.sourceFieldId === field.id
  );
  const targetFields =
  currentForm?.fields?.filter((f) => f.id !== field.id) || [];
    ...
    if (hasArrayOptions(typedField) || hasCheckboxOptions(typedField)) {
      ...
      if (Array.isArray(opts)) {
        if (opts.length > 0 && typeof opts[0] === "object") {
          return (opts as unknown as {label: string;value: string;}[]).map(
            (o) => o.value || o.label
          );
        }
  ...
  return (
    <div className="space-y-6">      <div className="space-y-4">        {fieldConditions.map((condition) =>
            ...
            className="absolute top-2 right-2 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <Select
                ...
                updateCondition(condition.id, { value })
                    ...
                    <SelectTrigger className="h-8 text-xs bg-white">                  <SelectValue
                    placeholder={t("builder.logic.placeholder_value")} />
              ...
              <Input
                ...
                placeholder={
                ["IS_EMPTY", "IS_NOT_EMPTY"].includes(condition.operator) ?
                t("builder.logic.placeholder_no_value") :
                t("builder.logic.placeholder_value_text")
                } />
              }            </div>          </div>          <div className="flex items-center gap-2">            <span className="text-xs font-medium text-gray-700">              {t("builder.logic.label_then")}            </span>          </div>          <div className="grid grid-cols-3 gap-2">            <div className="col-span-1 space-y-1">              <Label className="text-xs text-gray-500">                {t("builder.logic.label_action")}              </Label>              <Select
                value={condition.action}
                onValueChange={(value) =>
                updateCondition(condition.id, { action: value })
                }>
                  <SelectTrigger className="h-8 text-xs font-semibold text-blue-600 bg-blue-50 border-blue-100">                  <SelectValue />                </SelectTrigger>                <SelectContent>                  <SelectItem value="SHOW">                    {t("builder.logic.action_show")}                  </SelectItem>                  <SelectItem value="HIDE">                    {t("builder.logic.action_hide")}                  </SelectItem>                </SelectContent>              </Select>            </div>            <div className="col-span-2 space-y-1">              <Label className="text-xs text-gray-500">                {t("builder.logic.label_target")}              </Label>              <Select
                value={condition.targetFieldId}
                onValueChange={(value) =>
                updateCondition(condition.id, { targetFieldId: value })
                }>
                  <SelectTrigger className="h-8 text-xs bg-white">                  <SelectValue
                    placeholder={t("builder.logic.placeholder_target")} />
                  </SelectTrigger>                <SelectContent>                  {targetFields.map((f) =>
        ...
        }      </div>      <div className="bg-blue-50 p-4 rounded-lg text-xs text-blue-600 space-y-1">        <p className="font-semibold">{t("builder.logic.guide_title")}</p>        <ul className="list-disc pl-4 space-y-1">          <li>            <strong>{t("builder.logic.action_show")}:</strong>{" "}            {t("builder.logic.guide_show")}          </li>          <li>            <strong>{t("builder.logic.action_hide")}:</strong>{" "}            {t("builder.logic.guide_hide")}          </li>        </ul>      </div>    </div>);
};