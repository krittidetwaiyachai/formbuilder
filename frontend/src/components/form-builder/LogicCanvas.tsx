  ...
  return (
    <div className="py-6 px-4">      <div className="max-w-2xl mx-auto">        {}        <div className="flex items-center justify-between mb-8">          <div className="flex items-center gap-3">            {}            <div className="relative group/back">              {}              <div className="absolute -inset-[1px] bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 rounded-2xl opacity-75 blur-sm group-hover/back:opacity-100 group-hover/back:blur-md transition-all duration-500 animate-gradient-x" />              {}              <div className="absolute inset-0 rounded-2xl overflow-hidden">                <div className="absolute inset-0 opacity-0 group-hover/back:opacity-100 transition-opacity duration-300">                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-0 h-0 group-hover/back:w-32 group-hover/back:h-32 bg-white/20 rounded-full transition-all duration-700 ease-out" />                </div>              </div>              {}              <button
                ...
                {}                <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-transparent opacity-50" />                {}                <div className="absolute -inset-x-20 -top-20 h-40 bg-gradient-to-b from-white/30 via-white/10 to-transparent rotate-12 group-hover:translate-x-full group-hover:translate-y-full transition-transform duration-1000 ease-out" />                {}                <svg
                  className="absolute top-1 right-2 w-3 h-3 text-yellow-200 animate-spin-slow opacity-80"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  style={{ animationDuration: "4s" }}>
                  <path d="M12 0L14.59 8.41L24 12L14.59 15.59L12 24L9.41 15.59L0 12L9.41 8.41L12 0Z" />                </svg>                <svg
                  className="absolute bottom-1.5 left-2 w-2.5 h-2.5 text-pink-200 animate-spin-slow opacity-70"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  style={{ animationDuration: "5s", animationDelay: "0.5s" }}>
                  <path d="M12 0L14.59 8.41L24 12L14.59 15.59L12 24L9.41 15.59L0 12L9.41 8.41L12 0Z" />                </svg>                <svg
                  className="absolute top-2.5 left-1 w-2 h-2 text-purple-200 animate-spin-slow opacity-60"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  style={{ animationDuration: "3.5s", animationDelay: "1s" }}>
                  <path d="M12 0L14.59 8.41L24 12L14.59 15.59L12 24L9.41 15.59L0 12L9.41 8.41L12 0Z" />                </svg>                <svg
                  className="absolute bottom-2 right-1.5 w-2 h-2 text-blue-200 animate-spin-slow opacity-75"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  style={{ animationDuration: "4.5s", animationDelay: "0.3s" }}>
                  <path d="M12 0L14.59 8.41L24 12L14.59 15.59L12 24L9.41 15.59L0 12L9.41 8.41L12 0Z" />                </svg>                <svg
                  className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 text-white animate-spin-slow opacity-50"
                  viewBox="0 0 24 24"
                  fill="currentColor"
              ...
              {}              <div className="flex items-center justify-between mb-6">                <div className="flex items-center gap-3">                  {isEditing ?
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      {t("builder.logic.and")}                    </button>                    <button
                      onClick={() =>
                      updateLogicRule(rule.id, { logicType: "or" })
                      }
                      className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${
                      rule.logicType === "or" ?
                      "bg-white text-gray-900 shadow-sm" :
                  ...
                  <div key={action.id} className="flex items-center gap-2">                      <Select
                      ...
                      updateRuleAction(rule.id, action.id, {
                        ...
                        filter((f) => {
                          ...
                          return false;
                          const otherActions = rule.actions.filter(
                            (a) => a.id !== action.id
                          );
                          if (otherActions.some((a) => a.fieldId === f.id))
                          return false;
                          return true;
                        }).
                        map((f) =>
