...
export const FieldLabel: React.FC<FieldLabelProps> = ({
    ...
    const [subLabelHtml, setSubLabelHtml] = useState({ __html: sanitize((field.options?.subLabel as string) || '') || (isSelected ? t('common.sublabel') : '') });

    useEffect(() => {
        const currentText = subLabelRef.current?.textContent;
        const newSubLabel = (field.options?.subLabel as string) || (isSelected ? t('common.sublabel') : '');
    ...
    return (
        <div className={`${isRowLayout ? 'flex items-start gap-6 relative' : ''}`}>
             <div className={`${isRowLayout ? 'w-48 flex-shrink-0 pt-3' : 'mb-3'} ${labelAlignment === 'RIGHT' ? 'text-right' : ''} ${isCenterAligned ? 'text-center' : ''}`}>
                ...
                <div className={`flex-1 min-w-0 w-full max-w-full pb-3 scrollbar-visible ${field.type === FieldType.GROUP ? 'overflow-visible' : 'overflow-x-auto'}`}>
                     {children}
                     {![FieldType.HEADER, FieldType.PARAGRAPH, FieldType.DIVIDER, FieldType.SUBMIT, FieldType.PAGE_BREAK, FieldType.GROUP, FieldType.SECTION_COLLAPSE].includes(field.type) && (isSelected || ((field.options?.subLabel as string) && (field.options?.subLabel as string) !== t('common.sublabel') && (field.options?.subLabel as string) !== 'Sublabel')) && (
                       <div
                       ref={subLabelRef}
