...
export default function FormBuilderHeader({
  ...
  const { undo, redo, historyIndex, history, setCurrentForm } = useFormStore();
  ...
  null;
  const hasSaveError = message?.type === "error";
  const status = saving ? "saving" : hasSaveError ? "error" : hasUnsavedChanges ? "unsaved" : "saved";
  ...
  const handleCollaboratorUpdate = async () => {
    ...
    try {
      const response = await api.get<CollaboratorAccessResponse>(`/forms/${currentForm.id}/collaborator-access`);
      const latestForm = useFormStore.getState().currentForm;
      if (!latestForm || latestForm.id !== currentForm.id) {
        return;
      }
      setCurrentForm({
        ...latestForm,
        createdBy: response.data.owner || latestForm.createdBy,
        collaborators: response.data.collaborators || latestForm.collaborators || []
      });
    } catch (error) {
      console.error("Failed to sync collaborators in builder header:", error);
    }
  };
  return (
