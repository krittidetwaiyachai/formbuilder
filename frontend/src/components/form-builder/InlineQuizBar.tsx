...
export default function InlineQuizBar({
  field,
  currentForm,
  allFields,
  onUpdate
  ...
  const [localScore, setLocalScore] = useState<string | number>(
    field.score || 0
  );
  useEffect(() => {
  ...
  const getAnswerDisplay = () => {
    ...
    if (field.type === FieldType.CHECKBOX) {
      ...
      `${answers.length} ${t("builder.quiz.options")}` :
      answers[0];
    }
    if (answers.length > 1) {
      return `${answers.length} ${t("builder.quiz.answers")}`;
    }
    return answers[0].length > 20 ?
    answers[0].substring(0, 20) + "..." :
    answers[0];
  };
  return (
          ...
          onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">              <label className="text-xs font-semibold text-gray-700">                {t("builder.quiz.score")}              </label>              <button
              ...
              onBlur={() => {
                if (
                localScore === "" ||
                isNaN(parseInt(localScore as string, 10)))
                {
                  setLocalScore(field.score || 0);
                } else {
                  setLocalScore(field.score || 0);
