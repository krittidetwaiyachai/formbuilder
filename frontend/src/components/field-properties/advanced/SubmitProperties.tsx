...
import { Field, SubmitField, SubmitFieldOptions } from '@/types';
...
export const SubmitProperties: React.FC<SubmitPropertiesProps> = ({ field: rawField, updateField, duplicatesField }) => {
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState<'general' | 'options' | 'advanced'>('general');

  const field = rawField as unknown as SubmitField;
  const options = field.options || {};

  const handleUpdate = (updates: Partial<SubmitField>) => {
    updateField(field.id, updates);
  };

  const handleOptionUpdate = (key: keyof SubmitFieldOptions, value: unknown) => {
    handleUpdate({
      options: {
        ...options,
        [key]: value,
      } as SubmitFieldOptions,
    });
  };

  return (
    <div className="space-y-4">
      {/* <h3 className="font-semibold mb-2">{t('builder.properties.submit_button')}</h3> */}
                  ...
                  <button
                    ...
                    onClick={() => handleOptionUpdate('buttonAlign', align)}
                     className={`flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors ${
                      (options.buttonAlign === align) || (!options.buttonAlign && align === 'LEFT') // Default to LEFT if undefined
                        ? 'bg-black text-white border-black'
                        : 'bg-white text-black border-gray-400 hover:bg-gray-50'
                    }`}
                  >
                    {align === 'LEFT' ? t('builder.properties.left') : align === 'CENTER' ? t('builder.properties.center') : t('builder.properties.right')}
                  </button>
                ))}
              </div>
            </div>
