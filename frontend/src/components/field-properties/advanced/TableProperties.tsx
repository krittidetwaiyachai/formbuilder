...
import type { Field, TableField, Column, TableFieldOptions } from "@/types";
...
export function TableProperties({
  ...
  const columns: Column[] = typedField.options?.columns || [
  { id: "c1", label: t("builder.properties.column") + " 1" },
  { id: "c2", label: t("builder.properties.column") + " 2" }];
  const options: TableFieldOptions = typedField.options || {};
  const allowAddRow =
  options.allowAddRow !== undefined ? options.allowAddRow : true;
  ...
  const addColumn = () => {
    ...
    {
      id: newId,
      label: `${t("builder.properties.column")} ${columns.length + 1}`
    }]
    );
  };
  ...
  const applyTemplate = (type: string) => {
    if (type === "person") {
      ...
      {
        id: `c${Date.now()}_1`,
        label: t("builder.properties.street_address_1")
      },
