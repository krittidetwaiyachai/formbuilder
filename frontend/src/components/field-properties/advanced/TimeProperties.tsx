...
import type { Field, TimeField, TimeFieldOptions } from "@/types";
...
export const TimeProperties: React.FC<TimePropertiesProps> = ({
  field: rawField,
  updateField,
  duplicatesField
}) => {
    ...
    <div className="space-y-4">      {}      <PropertiesTabs activeTab={activeTab} setActiveTab={setActiveTab} />      {activeTab === "general" &&
      <div className="space-y-4">          {}          <div>            <label className="block text-sm font-medium text-black mb-1">              {t("builder.properties.field_label")}            </label>            <input
            ...
            <button
              ...
              onChange={(e) =>
              handleOptionUpdate("subLabelHour", e.target.value)
              }
              ...
              onChange={(e) =>
              handleOptionUpdate("subLabelMinutes", e.target.value)
              }
      ...
      <div className="space-y-6">          {}          <div>            <label className="block text-sm font-medium text-black mb-2">              {t("builder.properties.time_format")}            </label>            <div className="flex rounded-md overflow-hidden border border-gray-300">              {["24HOUR", "AMPM"].map((fmt) =>
            ...
            <button
              ...
              className={`flex-1 px-3 py-2 text-xs font-bold transition-colors ${
              validation.limitTime === mode ||
              !validation.limitTime && mode === "BOTH" ?
              "bg-black text-white" :
              "bg-gray-100 text-gray-700 hover:bg-gray-200"}`
              }>
                  {mode === "BOTH" ?
              t("builder.properties.limit_time_both") :
              mode === "AM" ?
              t("builder.properties.limit_time_am") :
              t("builder.properties.limit_time_pm")}                </button>
            ...
            <button
              ...
              className={`flex-1 px-3 py-2 text-xs font-bold transition-colors ${
              options.defaultTime === mode ||
              !options.defaultTime && mode === "NONE" ?
              "bg-black text-white" :
              "bg-gray-100 text-gray-700 hover:bg-gray-200"}`
              }>
                  {mode === "NONE" ?
              t("builder.properties.default_time_none") :
              mode === "CURRENT" ?
              t("builder.properties.default_time_current") :
              t("builder.properties.default_time_custom")}                </button>
          ...
          <div className="flex gap-2 items-center">                <input
              ...
              onChange={(e) =>
              handleOptionUpdate("customDefaultTime", e.target.value)
              } />
              </div>
              ...
              onChange={(e) =>
              handleOptionUpdate("timeRange", e.target.checked)
              }
              className="sr-only peer" />
