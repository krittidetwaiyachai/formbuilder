...
export function LongTextProperties({
  field,
  updateField,
  duplicatesField
}: LongTextPropertiesProps) {
  ...
  const handleOptionUpdate = <K extends keyof TextareaFieldOptions,>(
  key: K,
  value: TextareaFieldOptions[K]) =>
  {
    handleUpdate({
      options: {
        ...options,
  ...
  const handleValidationUpdate = <K extends keyof LongTextValidation,>(
  key: K,
  value: LongTextValidation[K]) =>
  {
    handleUpdate({
      validation: {
        ...validation,
    ...
    <>      <PropertiesTabs activeTab={activeTab} setActiveTab={setActiveTab} />      <div className="space-y-4">        {activeTab === "general" &&
        ...
        <div className="space-y-6">            <div>              <label className="block text-sm font-medium text-black mb-1">                {t("builder.properties.width")}              </label>              <label className="relative inline-flex items-center cursor-pointer">                <input
            ...
            <div className="mt-2">                  <input
                ...
                onChange={(e) =>
                handleOptionUpdate("rows", parseInt(e.target.value))
                }
                className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text"
                ...
                onChange={(e) =>
                handleValidationUpdate("hasEntryLimits", e.target.checked)
                }
                className="sr-only peer" />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300 after:ease-in-out after:shadow-sm peer-checked:bg-black"></div>              </label>              {validation.hasEntryLimits &&
            <div className="mt-2 space-y-2">                  <div className="flex gap-2">                    <div className="flex-1">                      <label className="text-xs text-gray-500 mb-1 block">                        {t("builder.properties.min")}                      </label>                      <input
                    type="number"
              ...
              <button
                ...
                !options.editorMode && mode === "PLAIN_TEXT" ?
                "bg-black text-white" :
                "bg-white text-black hover:bg-gray-50"}`
                }>
                    {mode === "PLAIN_TEXT" ?
                t("builder.properties.plain_text") :
                t("builder.properties.rich_text")}                  </button>
              )}              </div>              <p className="mt-1 text-xs text-gray-500">                {t("builder.properties.editor_mode_desc")}              </p>            </div>            <div>              <label className="block text-sm font-medium text-black mb-1">                {t("builder.properties.validation")}              </label>              <Select
              value={validation.type || "None"}
              onValueChange={(val) =>
              handleValidationUpdate(
                "type",
                val as LongTextValidation["type"]
              )
              }>
                <SelectTrigger className="w-full bg-white border-gray-400">                  <SelectValue
                  placeholder={t("builder.properties.select_validation")} />
                </SelectTrigger>                <SelectContent>                  <SelectItem value="None">                    {t("builder.properties.validation_none")}                  </SelectItem>                  <SelectItem value="Email">                    {t("builder.properties.validation_email")}                  </SelectItem>                  <SelectItem value="URL">                    {t("builder.properties.validation_url")}                  </SelectItem>                  <SelectItem value="Alphabetic">                    {t("builder.properties.validation_alphabetic")}                  </SelectItem>                  <SelectItem value="Alphanumeric">                    {t("builder.properties.validation_alphanumeric")}                  </SelectItem>                  <SelectItem value="Numeric">                    {t("builder.properties.validation_numeric")}                  </SelectItem>                  <SelectItem value="Regex">                    {t("builder.properties.validation_regex")}                  </SelectItem>                </SelectContent>              </Select>              <p className="mt-1 text-xs text-gray-500">                {t("builder.properties.validation_desc")}              </p>            </div>          </div>
        }        {activeTab === "advanced" &&
        <div className="space-y-6">            <div>              <label className="block text-sm font-medium text-black mb-1">                {t("builder.properties.placeholder")}              </label>              <input
              ...
              onChange={(e) =>
              handleOptionUpdate("hoverText", e.target.value)
              }
              rows={2}
              className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text" />
              ...
              onChange={(e) =>
              handleOptionUpdate("defaultValue", e.target.value)
              }
              rows={3}
              className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text" />
              <p className="mt-1 text-xs text-gray-500">                {t("builder.properties.default_value_desc")}              </p>            </div>            <div>              <label className="block text-sm font-medium text-black mb-1">                {t("builder.properties.read_only")}              </label>              <label className="relative inline-flex items-center cursor-pointer">                <input
                ...
                onChange={(e) =>
                handleOptionUpdate("shrink", e.target.checked)
                }
                className="sr-only peer" />
                ...
                onChange={(e) =>
                handleOptionUpdate("hidden", e.target.checked)
                }
                className="sr-only peer" />
