...
export const NumberProperties: React.FC<NumberPropertiesProps> = ({
  field,
  updateField,
  duplicatesField,
}) => {
  ...
  const handleValidationUpdate = <K extends keyof NumberValidation>(
    key: K,
    value: NumberValidation[K],
  ) => {
    handleUpdate({
      validation: {
        ...validation,
  ...
  const handleOptionUpdate = <K extends keyof NumberFieldOptions>(
    key: K,
    value: NumberFieldOptions[K],
  ) => {
    handleUpdate({
      options: {
        ...options,
            ...
            {options.width === "FIXED" && (
              <div className="mt-2">
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  {t("builder.properties.custom_width")}
                </label>
                <input
                  type="number"
                  value={options.customWidth || ""}
                  onChange={(e) =>
                    handleOptionUpdate("customWidth", parseInt(e.target.value))
                  }
          ...
          <div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  handleValidationUpdate("entryLimits", e.target.checked)
                }
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300 after:ease-in-out after:shadow-sm peer-checked:bg-black"></div>
              ...
              <div className="mt-4 flex gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleValidationUpdate("min", parseFloat(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleValidationUpdate("max", parseFloat(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-black bg-white select-text text-sm"
                  />
                </div>
