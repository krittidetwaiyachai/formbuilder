                ...
                return (
                  <div key={i} className="flex items-center gap-1.5 bg-white px-2 py-1 rounded border border-gray-200 shadow-sm">
                    ...
                    <span className="text-[10px] text-gray-400 bg-gray-50 px-1 rounded uppercase tracking-wider">
                      {typeof f.type === 'string' ? getFieldTypeName(f.type, t, i18n) : 'FIELD'}
                    </span>
                  </div>
                );
              })}
                 ...
                 return (
                  ...
                  <div className="space-y-1">
                    {(f.changes || []).map((c: ChangeItem, idx: number) => {
                       ...
                       if (c.property === 'rows' && !beforeValue) beforeValue = 4;
                       if (c.property === 'customWidth' && !beforeValue) beforeValue = 300;
                       if ((c.property === 'validation.separator' || c.property === 'separator') && !beforeValue) beforeValue = '/';
                       ...
                       if ((c.property === 'validation.maxLength' || c.property === 'maxLength') && !beforeValue) beforeValue = 100;
                       if ((c.property === 'validation.imagePosition' || c.property === 'imagePosition') && !beforeValue) beforeValue = 'CENTER';
