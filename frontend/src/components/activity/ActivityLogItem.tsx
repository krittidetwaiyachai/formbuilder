...
export default function ActivityLogItem({
  ...
  const getLogTitle = (log: ActivityLog) => {
    switch (log.action) {
      case "CREATED":
        return (
          <span className="text-gray-900">{t("activity.log.created")}</span>
        );
      case "DELETED":
        return (
          <span className="text-rose-600">{t("activity.log.deleted")}</span>
        );
      case "UPDATED":
        return (
          <span className="text-gray-900">{t("activity.log.updated")}</span>
        );
      default:
        return (
          <span className="text-gray-900">{log.action.toLowerCase()}</span>
        );
    }
  };
          ...
          <div className="flex-1">
            <p className="text-base text-gray-900">
              <span className="font-bold hover:text-indigo-600 transition-colors cursor-default">
                {typeof log.user.firstName === "string"
                  ? log.user.firstName
                  : "User"}{" "}
                {typeof log.user.lastName === "string" ? log.user.lastName : ""}
              </span>{" "}
              {getLogTitle(log)}
            </p>
          </div>
        ...
        <div className="pl-4 border-l-2 border-gray-100">
          {log.action === "CREATED" && (
            <div className="text-sm text-gray-500 italic">
              {t("activity.started_journey", {
                title:
                  typeof log.details?.title === "string"
                    ? log.details.title
                    : t("common.untitled_form"),
              })}
            </div>
          )}
            ...
            (!settingsChanges || settingsChanges.length === 0) && (
              <div className="flex flex-wrap gap-2 mt-3">
                <span className="text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-100 flex items-center gap-1">
                  ...
                  {t("activity.changes.settings_modified")}
                </span>
                {changes
                  .filter((c: string) => c !== "fields")
                  .map((c: string, i: number) => (
                    <span
                      key={i}
                      className="text-xs text-gray-700 bg-white px-2 py-1 rounded border border-gray-200 shadow-sm"
                    >
