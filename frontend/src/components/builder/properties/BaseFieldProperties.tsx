...
export default function BaseFieldProperties({
  element,
  onUpdate
}: BaseFieldPropertiesProps) {
    ...
    <>
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label className="text-base font-medium">
              {t(
                "builder.properties.pdpa_title",
                "Collect Personal Data (PDPA)"
              )}
            </Label>
            <div className="text-[0.8rem] text-muted-foreground">
              {t(
                "builder.properties.pdpa_desc",
                "Data in this field will be encrypted before saving"
              )}
            </div>
          </div>
      ...
      <div className="space-y-4 pt-4 border-t border-gray-100">
        <h3 className="text-sm font-medium text-gray-700">
          {t("builder.properties.media", "Media (Image & Video)")}
        </h3>
        <div className="space-y-2">
      ...
      <div className="flex items-center space-x-2 pt-2">
        <input
          type="checkbox"
          id="shrink"
          checked={element.shrink || false}
          onChange={(e) => onUpdate({ shrink: e.target.checked })}
          className="h-4 w-4 rounded border-gray-300" />
        <Label htmlFor="shrink" className="cursor-pointer">
          {t("builder.properties.shrink", "Display as 50% width")}
        </Label>
      </div>
      <div className="flex items-center space-x-2 pt-2">
        <input
