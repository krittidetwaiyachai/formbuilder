...
export default function ThemeSelectionModal({ isOpen, onClose, currentTheme, onThemeSelect }: ThemeSelectionModalProps) {
  ...
  useSmoothScroll('theme-modal-scroll', { enabled: isOpen });
  React.useEffect(() => {
                          ...
                          <div className="absolute inset-0 bg-cover bg-center opacity-50" style={{ backgroundImage: `url(${theme.backgroundImage})` }} />
                            ...
                            style={{
                              ...
                              borderRadius: theme.borderRadius === 'small' ? '4px' : theme.borderRadius === 'medium' ? '8px' : theme.borderRadius === 'large' ? '12px' : '0px'
                                        ...
                                        {t('dashboard.theme.submit_preview', 'Submit')}                        </div>                      </div>                      {}                      <div className="p-3" style={{ backgroundColor: cardBg }}>                        <div className="flex items-center justify-between">                          <span className="font-semibold capitalize" style={{ color: cardTextColor }}>{name}</span>                          {}                          <span className="text-[10px] uppercase tracking-wider" style={{ fontFamily: theme.fontFamily, color: cardTextColor, opacity: 0.7 }}>Aa</span>                        </div>                        <div className="flex gap-1 mt-2">                          <div className="w-4 h-4 rounded-full border" style={{ backgroundColor: theme.primaryColor, borderColor: isDarkTheme ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)' }} title="Primary" />                          <div className="w-4 h-4 rounded-full border" style={{ backgroundColor: theme.backgroundColor, borderColor: isDarkTheme ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)' }} title="Background" />                          <div className="w-4 h-4 rounded-full border" style={{ backgroundColor: theme.textColor, borderColor: isDarkTheme ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)' }} title="Text" />                        </div>                      </div>                      {}                      {isThemeEqual(workingTheme, theme) &&
                        <div className="absolute top-2 right-2 w-7 h-7 bg-black rounded-full flex items-center justify-center text-white shadow-lg">                        <Check className="w-4 h-4" />                      </div>
                        }                    </button>);
                  })}              </div>            </div>          </div>        </div></div>        {}        <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-end gap-3 shrink-0">          <Button onClick={() => {onThemeSelect({ ...workingTheme, themeName: selectedThemeName });onClose();}} className="bg-black hover:bg-gray-800 text-white">{t('dashboard.theme.done', 'Done')}</Button>        </div>      </div>    </div>,
    document.body
  );
}