...
export default function DesignerElementCard({
  ...
  if (isDragging || isOverlay) {
    return (
      <div className="w-64 bg-white rounded-xl shadow-2xl p-4 border-2 border-primary/20 ring-4 ring-black/5 cursor-grabbing flex flex-col gap-2">        <div className="flex justify-center">          <div className="w-8 h-1 bg-gray-200 rounded-full" />        </div>        <div className="flex items-center gap-2">          <span className="font-medium text-base truncate text-primary">            {element.type}          </span>        </div>        <div className="h-8 w-full bg-gray-50 rounded border border-gray-100 flex items-center px-3 text-xs text-gray-400 font-medium select-none">          {element.type} Field        </div>      </div>);
  }
  return (
    <div className={cn("relative group", isSelected && "z-10")}>      <div
        className={cn(
          ...
          isSelected &&
          "border-[3px] shadow-xl scale-[1.01] ring-2 ring-black/5 z-20 pt-4 pb-2 px-3"
        )}
        ...
        <div
          className="absolute top-1 left-1/2 -translate-x-1/2 z-[50] cursor-grab active:cursor-grabbing w-full flex justify-center h-4"
          title="Drag to move">
