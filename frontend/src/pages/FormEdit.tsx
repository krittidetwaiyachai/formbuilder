...
export default function FormBuilderPage() {
  ...
  // Drag and Drop
  const { onDragEnd, onDragStart: originalOnDragStart, isDragging } = useFormDragAndDrop({
      id: id!,
      activeFields, 
      setActiveFields,
      updateForm,
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 text-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-md w-full border border-gray-100">
          ...
          <button onClick={() => navigate('/dashboard')} className="px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors font-medium w-full">{t('builder.back_to_dashboard')}</button>
        </div>
      </div>
    );
  }
              ...
              <SmoothScrollProvider targetId="builder-canvas-scroll-container">
                  ...
                  >
                        {activeSidebarTab === 'logic' ? (
                            ...
                            <>
                                {currentForm?.isQuiz && <QuizModeBanner />}
