...
export function useFormSubmission({
  ...
  const submitForm = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      const response = await api.post(`/public/forms/${form.id}/submissions`, {
        answers,
        respondentEmail:
      ...
      err.response?.data?.code === 'VERIFICATION_REQUIRED')
      {
        onVerificationRequired?.();
        toast({
          title: i18n.t('error.verification_required.title'),
          description: i18n.t('error.verification_required.message'),
          variant: 'warning'
        });
      } else {
        toast({
          title: i18n.t('error.submission_failed.title'),
          description: getAxiosErrorMessage(err, i18n.t('error.submission_failed.message')),
          variant: "error"
        });
      }
    } finally {
