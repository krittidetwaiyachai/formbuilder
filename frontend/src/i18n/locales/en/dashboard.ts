export default {
  "dashboard.welcome": "Welcome back, {{username}}",
  "dashboard.welcome_default": "Welcome back, Creator",
  "dashboard.welcome_back": "Welcome back",
  "dashboard.create_form": "Create New Form",
  "dashboard.search_placeholder": "Search forms...",
  "dashboard.loading": "Loading your dashboard...",
  "dashboard.filters.all": "All",
  "dashboard.filters.draft": "DRAFT",
  "dashboard.filters.published": "PUBLISHED",
  "dashboard.filters.archived": "ARCHIVED",
  "dashboard.folders": "Folders",
  "dashboard.move_to_folder": "Move to Folder",
  "dashboard.folder.all": "All",
  "dashboard.folder.ungrouped": "Ungrouped",
  "dashboard.new_folder": "New Folder",
  "dashboard.all_forms": "All Forms",
  "dashboard.ungrouped_forms": "Forms",
  "dashboard.empty.title": "No forms found",
  "dashboard.empty.no_search": "We couldn't find any forms matching your search.",
  "dashboard.empty.no_forms": "You haven't created any forms yet. Start your journey now!",
  "dashboard.empty.create_first": "Create your first form",
  "dashboard.empty.description": "Click the button above to create your first form.",
  "dashboard.form.responses": "response",
  "dashboard.form.responses_plural": "responses",
  "dashboard.form.views": "view",
  "dashboard.form.views_plural": "views",
  "dashboard.form.editors": "editor",
  "dashboard.form.editors_plural": "editors",
  "dashboard.form.owner": "Owner",
  "dashboard.form.shared": "Shared",
  "dashboard.form.no_description": "No description provided.",
  "dashboard.modal.folder_name": "Folder Name",
  "dashboard.modal.folder_color": "Choose Color",
  "dashboard.modal.create": "Create",
  "dashboard.modal.cancel": "Cancel",
  "dashboard.modal.save": "Save",
  "dashboard.context.edit": "Edit",
  "dashboard.context.preview": "Preview",
  "dashboard.context.analytics": "Analytics",
  "dashboard.context.activity": "Activity",
  "dashboard.context.copy_link": "Copy Link",
  "dashboard.context.duplicate": "Duplicate",
  "dashboard.context.collaborators": "Manage Access",
  "dashboard.context.delete": "Delete",
  "dashboard.confirm.delete_form.title": "Delete Form?",
  "dashboard.confirm.delete_form.description": "Are you sure you want to delete this form? This action cannot be undone.",
  "dashboard.confirm.delete_folder.title": "Delete Folder?",
  "dashboard.confirm.delete_folder.description": "Are you sure you want to delete this folder? Forms inside will be moved to All Forms.",
  "dashboard.confirm.delete": "Delete",
  "dashboard.confirm.delete_folder": "Delete Folder",
  "dashboard.confirm.cancel": "Cancel",
  "dashboard.toast.created": "Form created successfully.",
  "dashboard.toast.deleted": "Form deleted successfully.",
  "dashboard.toast.duplicated": "Form duplicated successfully.",
  "dashboard.toast.link_copied": "Link copied to clipboard!",
  "dashboard.toast.folder_created": "Folder created successfully.",
  "dashboard.toast.folder_deleted": "Folder deleted successfully.",
  "dashboard.toast.moved": "Form moved successfully.",
  "dashboard.toast.moved_desc": "The form has been moved to the new folder.",
  "dashboard.toast.error": "Error",
  "dashboard.toast.error_create": "Cannot create form at this moment.",
  "dashboard.toast.error_delete": "Failed to delete form. Please try again.",
  "dashboard.toast.error_duplicate": "Failed to duplicate form.",
  "dashboard.toast.error_folder_delete": "Failed to delete folder. Please try again.",
  "dashboard.drop_ungroup": "Drop here to ungroup",
  "dashboard.forms_count": "forms",
  "dashboard.context.rename": "Rename",
  "dashboard.prompt.new_folder_name": "New folder name:",
  "dashboard.modal.folder_placeholder": "e.g. Client Projects",
  "dashboard.context.group.main": "Main Actions",
  "dashboard.context.group.management": "Management",
  "dashboard.context.group.share": "Share & More",
  "dashboard.context.group.danger": "Danger Zone",
  "dashboard.collaborators.manage_access": "Manage Access",
  "dashboard.collaborators.add_people": "Add People",
  "dashboard.collaborators.email_placeholder": "Enter email address",
  "dashboard.collaborators.invite": "Invite",
  "dashboard.collaborators.invite_success": "Invitation sent successfully",
  "dashboard.collaborators.invite_error": "Failed to invite user",
  "dashboard.collaborators.message.invite_success": "Invitation sent successfully",
  "dashboard.collaborators.message.added_success": "Collaborator added successfully",
  "dashboard.collaborators.message.added_and_email_sent": "Collaborator added and invitation email sent",
  "dashboard.collaborators.message.added_but_email_not_sent": "Collaborator added, but invitation email was not sent",
  "dashboard.collaborators.message.already_collaborator": "User is already a collaborator",
  "dashboard.collaborators.message.owner_cannot_be_collaborator": "Form owner cannot be added as a collaborator",
  "dashboard.collaborators.message.only_owner_can_invite": "Only the form owner can add collaborators",
  "dashboard.collaborators.message.user_not_found": "User not found",
  "dashboard.collaborators.message.form_not_found": "Form not found",
  "dashboard.collaborators.message.invitation_accepted": "Invitation accepted successfully",
  "dashboard.collaborators.message.invitation_resent": "Invitation resent successfully",
  "dashboard.collaborators.message.invitation_revoked": "Invitation revoked successfully",
  "dashboard.collaborators.message.invitation_token_required": "Invitation token is required",
  "dashboard.collaborators.message.invitation_not_found": "Invitation not found",
  "dashboard.collaborators.message.invitation_invalid": "Invitation is no longer valid",
  "dashboard.collaborators.message.invitation_expired": "Invitation has expired",
  "dashboard.collaborators.message.invitation_revoked_by_owner": "This invitation has been revoked",
  "dashboard.collaborators.message.invitation_already_accepted": "This invitation has already been accepted",
  "dashboard.collaborators.message.invitation_different_email": "This invitation was sent to a different email",
  "dashboard.collaborators.message.email_required": "Email is required",
  "dashboard.collaborators.message.remove_permission_denied": "You do not have permission to remove collaborators",
  "dashboard.collaborators.pending_title": "Pending Invitations",
  "dashboard.collaborators.pending_empty": "No pending invitations",
  "dashboard.collaborators.pending_note": "This email has been invited and is waiting for acceptance",
  "dashboard.collaborators.resend": "Resend",
  "dashboard.collaborators.revoke": "Revoke",
  "dashboard.collaborators.current_access": "Current Access",
  "dashboard.collaborators.no_collaborators": "No collaborators found.",
  "dashboard.collaborators.owner": "OWNER",
  "dashboard.collaborators.you": "YOU",
  "dashboard.collaborators.no_email": "No email provided",
  "dashboard.collaborators.remove_access": "Remove access",
  "dashboard.collaborators.remove_confirm_title": "Remove Collaborator?",
  "dashboard.collaborators.remove_confirm": "Are you sure you want to remove this collaborator?",
  "dashboard.collaborators.remove_error": "Failed to remove user",
  "dashboard.collaborators.leave": "Leave",
  "dashboard.form.quiz_mode": "Quiz Mode",
  "dashboard.form.statistics": "Statistics",
  "dashboard.form.timeline": "Timeline",
  "dashboard.form.created": "Created",
  "dashboard.form.last_updated": "Last Updated",
  "dashboard.filters.title": "Filter & Sort",
  "dashboard.filters.status": "Status",
  "dashboard.filters.apply": "Show Results",
  "dashboard.sort.title": "Sort By",
  "dashboard.sort.newest": "Newest",
  "dashboard.sort.oldest": "Oldest",
  "dashboard.sort.alphabetical": "A-Z",
  "dashboard.folder.expand": "Expand",
  "dashboard.folder.collapse": "Collapse",
  "dashboard.folder.actions": "Actions",
  "dashboard.folder.empty": "This folder is empty",
  "dashboard.folder.count": "folder",
  "dashboard.folder.count_plural": "folders",
  "dashboard.modal.close": "Close",
  "dashboard.modal.creating": "Creating...",
  "dashboard.toast.error_folder_create": "Failed to create folder. Please try again.",
  "dashboard.stats.responses": "Responses",
  "dashboard.stats.views": "Views",
  "dashboard.theme.popular": "Popular Themes",
  "dashboard.theme.exclusive": "Exclusive Themes",
  "dashboard.theme.festival": "Festival Themes",
  "dashboard.theme.official": "Official Theme",
  "dashboard.theme.exclusive_badge": "EXCLUSIVE",
  "dashboard.theme.submit_preview": "Submit",
  "dashboard.theme.close": "Close",
  "dashboard.theme.done": "Done",
  "dashboard.theme.appearance": "Form Appearance",
  "dashboard.theme.customize_desc": "Customize how your form looks",
  "dashboard.form.updated_prefix": "Updated ",
  "dashboard.toast.error_load_folders": "Failed to load folders",
  "dashboard.toast.error_update_folder": "Failed to update folder",
  "dashboard.toast.error_move_form": "Failed to move form"
};
