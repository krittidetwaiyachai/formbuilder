...
BundlesService {
  ...
  async findAll(isActive?: boolean) {
    ...
    const allBundles = bundles.filter(
      (b) => !(b.options as unknown as BundleOptions)?.deleted
    );
  ...
  updateData: Partial<CreateBundleDto>)
  {
    ...
    const version = latestBundle ?
    latestBundle.version + 1 :
    bundle.version + 1;
    type UpsertBundleField = CreateBundleFieldDto & {id?: string;};
    const incomingFields = (fields as UpsertBundleField[] | undefined)?.map(
      (field, index) => ({
        ...field,
    ...
    const updatedBundle = await this.prisma.$transaction(async (tx) => {
      await tx.bundle.update({
        ...
        data: {
          ...
          options:
          bundleData.options as Prisma.InputJsonValue ??
          bundle.options as Prisma.InputJsonValue,
          version
        }
      });
      if (incomingFields) {
        const existingFieldIds = new Set(
          bundle.fields.map((field) => field.id)
        );
