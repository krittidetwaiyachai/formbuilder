...
import {
  authenticateSocket,
  WsAuthenticatedUser } from
'../common/guards/ws-auth.util';
...
CollaborationGateway implements
  OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    ...
    if (!user) {
      this.logger.warn(
        `Unauthenticated collaboration connection rejected: ${client.id}`
      );
    ...
    @ConnectedSocket()client: Socket)
  {
    ...
    const usedColors = new Set(
      Array.from(formUsers!.values()).map((activeUser) => activeUser.color)
    );
    ...
    this.broadcastActiveUsers(formId);
    this.logger.log(
      `User ${authenticatedUser.email} joined form ${formId} (Socket: ${client.id})`
    );
  }
