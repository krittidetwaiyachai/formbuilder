...
ActivityLogService {
  ...
  private buildDeviceInfo(userAgent?: string) {
    ...
    'Unknown';
    const browser =
    uaLower.includes('edg/') || uaLower.includes('edge/') ?
    'Edge' :
    ...
    'Unknown';
    const deviceType =
    uaLower.includes('mobile') || os === 'Android' || os === 'iOS' ?
    'mobile' :
    'desktop';
    return {
      type: deviceType,
  ...
  userId?: string)
  {
    ...
    if (action && action !== 'ALL') {
      if (action === 'CREATED') {
        ...
        {
          AND: [
          { details: { path: ['updatedFields'], not: [] } },
          { details: { path: ['updatedFields'], not: null } }]
        },
        {
          AND: [
  ...
  async getFormEditors(formId: string) {
    const editors = await this.prisma.$queryRaw<
      Array<{
        id: string;
