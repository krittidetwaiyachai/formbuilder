...
import { RedisRateLimitService } from '../form-security/redis-rate-limit.service';
...
AuthController {
  ...
  {}
  private async consumeLoginRateLimit(ipAddress: string, email?: string) {
    const policy = await this.systemSettingsService.getRateLimitSettings();
    const normalizedIp = ipAddress?.trim() || 'unknown';
    await this.rateLimitService.consume(
      `auth:login:ip:${normalizedIp}`,
      policy.authLoginLimit,
      policy.authLoginWindowSeconds
    );
  ...
  private async verifyAuthCaptcha(
  ...
  ipAddress?: string)
  {
    ...
    const isVerified = await this.turnstileService.verifyToken(
      captchaToken || '',
      ipAddress
    );
    if (!isVerified) {
  ...
  private isRefreshCookieEnabled() {
    return (
      this.configService.get<string>('AUTH_REFRESH_COOKIE_ENABLED') !== 'false');
  }
  private isRefreshCookieOnlyMode() {
    return (
      this.isRefreshCookieEnabled() &&
      this.configService.get<string>('AUTH_REFRESH_COOKIE_ONLY') !== 'false');
  }
  ...
  private getRefreshCookieSameSite(): 'lax' | 'strict' | 'none' {
    const raw = (
    this.configService.get<string>('AUTH_REFRESH_COOKIE_SAMESITE') || 'lax').
    toLowerCase();
  ...
  private writeRefreshTokenCookie(
  res: Response,
  refreshToken: string,
  ...
  })
  {
    if (payload.refresh_token) {
      this.writeRefreshTokenCookie(
        res,
        payload.refresh_token,
        payload.refresh_token_max_age_ms
      );
  ...
  async googleLogin(
    ...
    @Res({ passthrough: true })res: Response)
  {
    await this.consumeLoginRateLimit(ipAddress);
  ...
  {
    const refreshToken =
    body.refresh_token || this.readRefreshTokenCookie(req) || '';
    ...
    @Body('email')targetEmail: string)
  {
    ...
    return this.authService.requestEmailVerification(userId, targetEmail);
  }
  @Post('email/verify')
  @Throttle({ default: { limit: 5, ttl: 60000 } })
    ...
    @Body('code')code: string)
  {
    ...
    return this.authService.verifyEmailOtp(userId, code);
  }
}