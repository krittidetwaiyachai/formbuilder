...
ResponsesStatsService {
  ...
  private async getResponseTrend(
  formId: string,
  month?: string)
  : Promise<TrendItem[]> {
  ...
  formId: string,
  form: {
    fields: {id: string;type: string;label: string;options: unknown;}[];
  })
  : Promise<FieldStatResult[]> {
    ...
    'PAGE_BREAK',
    'SUBMIT'];
    const analyzableFields = form.fields.filter(
      (f) => !skipTypes.includes(f.type)
    );
    if (analyzableFields.length === 0) return [];
    const simpleFields = analyzableFields.filter(
      (f) => !['MATRIX', 'TABLE'].includes(f.type)
    );
    const complexFields = analyzableFields.filter((f) =>
    ['MATRIX', 'TABLE'].includes(f.type)
    );
    const stats: FieldStatResult[] = [];
    if (simpleFields.length > 0) {
      const fieldIds = simpleFields.map((f) => f.id);
      const rawCounts = await this.prisma.$queryRaw<
        {fieldId: string;value: string;count: bigint;}[]>
      `
            ...
            try {
              ...
              if (typeof parsed === 'object' && parsed !== null) {
                ...
                if (rowVal) {
                  ...
                  rowResponses.push(...rowVal.map(String));else
  ...
  : Promise<QuizStatsResult | null> {
    ...
    const distRaw = await this.prisma.$queryRaw<
    ...
    const questionStatsRaw =
    quizFields.length > 0 ?
    await this.prisma.$queryRaw<
      {fieldId: string;correct: bigint;incorrect: bigint;}[]>
    `
          SELECT
            ra."fieldId",
    ...
    return {
      ...
      passRate:
      totalCount > 0 ? Number(row.pass_count) / totalCount * 100 : 0,
