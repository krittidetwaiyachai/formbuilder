...
ResponsesService {
  ...
  private getExportLockTtlSeconds() {
    const parsed = Number.parseInt(
      this.configService.get<string>('EXPORT_LOCK_TTL_SECONDS') || '',
      10
    );
  ...
  private getExportResultTtlSeconds() {
    const parsed = Number.parseInt(
      this.configService.get<string>('EXPORT_RESULT_TTL_SECONDS') || '',
      10
    );
    if (!Number.isFinite(parsed) || parsed <= 0) {
      return 900;
  ...
  private getExportIdempotencyRedisKey(
  ownerId: string,
  formId: string,
  ...
  private async readExportIdempotencyJobId(
  ownerId: string,
  formId: string,
  idempotencyKey: string)
  {
  ...
  {
    const redis = this.getExportRedisClient();
    const key = this.getExportIdempotencyRedisKey(
      ownerId,
      formId,
      idempotencyKey
    );
    ...
    if (redis) {
      ...
      if (response !== 'OK') {
        throw new ConflictException(
          'An export is already processing for this form.'
        );
      }
      return;
    }
  ...
  sort: 'asc' | 'desc' = 'desc')
  {
    ...
    const decryptedResponses = responses.map((response) => ({
      ...
      normalizedRespondentEmail: canViewPii ?
      response.normalizedRespondentEmail :
      null,
  ...
  canViewPii: boolean = false)
  {
    ...
    try {
      ...
      if (normalizedIdempotencyKey) {
        await this.writeExportIdempotencyJobId(
          userId,
          formId,
          normalizedIdempotencyKey,
          jobId
      ...
      await this.releaseExportLock(userId, formId, lockToken).catch(
        () => undefined
      );
      throw error;
    }
  ...
  : Promise<{filePath: string;filename: string;}> {
    ...
    if (!fs.existsSync(filePath)) {
      throw new NotFoundException(
        'Export file not found, it might have been deleted.'
      );
    }
  ...
  })
  {
    ...
    form.fields.forEach((f) => {
      if (
      ['HEADER', 'PARAGRAPH', 'DIVIDER', 'PAGE_BREAK', 'SUBMIT'].includes(
        f.type
      ))
      {
        return;
      ...
      if (f.type === 'MATRIX') {
        const matrixOptions = f.options as unknown as {
    ...
    const piiFieldIds = new Set(
      form.fields.filter((f) => f.isPII).map((f) => f.id)
    );
            ...
            if (field.type === 'MATRIX') {
              ...
              if (matrixOptions?.rows && Array.isArray(matrixOptions.rows)) {
                ...
                if (value === '[object Object]') {
                  matrixOptions.rows.forEach(() =>
                  row.push(escapeCsv('Error: Invalid Data'))
                  );
                } else {
                  const colMap = new Map(
                    (matrixOptions.columns || []).map((c) => [c.id, c.label])
                  );
                  matrixOptions.rows.forEach((matrixRow) => {
                    ...
                    if (cellValue) {
                      ...
                      cellValue.
                      map((v) => colMap.get(String(v)) || v).
                      join(', ') :
                      (colMap.get(String(cellValue)) ||
                      cellValue) as string;
                    }
