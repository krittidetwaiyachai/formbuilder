...
SystemSettingsService {
  ...
  fallback = DEFAULT_BRANDING_PRIMARY_COLOR)
  {
  ...
  private normalizeInviteExpiryDays(
  value: unknown,
  fallback = DEFAULT_INVITE_EXPIRY_DAYS)
  {
    const parsed = Number.parseInt(String(value ?? ''), 10);
    if (!Number.isFinite(parsed)) {
      return fallback;
  ...
  private getDefaultSupportEmail() {
    return (
      this.normalizeString(this.configService.get<string>('SUPPORT_EMAIL')) ||
      DEFAULT_SUPPORT_EMAIL);
  }
  private getDefaultSupportLineId() {
    return (
      this.normalizeString(this.configService.get<string>('SUPPORT_LINE')) ||
      DEFAULT_SUPPORT_LINE_ID);
  }
  ...
  private buildContactSettings(settings: {
    supportEmail?: string | null;
    supportLineId?: string | null;
  }) {
    return {
      supportEmail:
      this.normalizeString(settings.supportEmail) ||
      this.getDefaultSupportEmail(),
      supportLineId:
      this.normalizeString(settings.supportLineId) ||
      this.getDefaultSupportLineId()
    };
  }
  private buildBrandingSettings(settings: {
    ...
    return {
      appName:
      this.normalizeString(settings.brandingAppName) || DEFAULT_APP_NAME,
      logoUrl: this.normalizeString(settings.brandingLogoUrl) || '',
      primaryColor: this.normalizeHexColor(
  ...
  private buildPasswordPolicySettings(settings: {
    ...
    return {
      minLength: this.normalizeInteger(
  ...
  private buildRateLimitSettings(settings: {
    ...
    return {
      authLoginLimit: this.normalizeInteger(
        settings.rateAuthLoginLimit,
  ...
  private buildRetentionSettings(settings: {
    ...
    return {
      autoCleanupEnabled:
      settings.retentionAutoCleanupEnabled ??
      DEFAULT_RETENTION_AUTO_CLEANUP_ENABLED,
      responsesDays: this.normalizeOptionalDays(
        settings.retentionResponsesDays,
        null
      ),
  ...
  private buildBackupSettings(settings: {
    ...
    return {
      ...
      intervalHours: this.normalizeInteger(
        settings.backupIntervalHours,
        DEFAULT_BACKUP_INTERVAL_HOURS,
        1,
        24 * 30
      ),
      retentionDays: this.normalizeInteger(
      ...
      directory:
      this.normalizeString(settings.backupDirectory) ||
      this.getDefaultBackupDirectory(),
  ...
  async getPublicSettings() {
    ...
    return {
      email: {
        ...
        this.normalizeString(this.configService.get<string>('SMTP_PASS')) !==
        null,
        isConfigured:
  ...
  async updateEmailSettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeString(payload.smtpHost) :
        existing.smtpHost,
        smtpPort:
        ...
        existing.smtpPort,
        smtpSecure:
        typeof payload.smtpSecure === 'boolean' ?
        payload.smtpSecure :
        existing.smtpSecure,
        smtpUser:
        payload.smtpUser !== undefined ?
        this.normalizeString(payload.smtpUser) :
        existing.smtpUser,
        smtpFrom:
        payload.smtpFrom !== undefined ?
        this.normalizeString(payload.smtpFrom) :
        existing.smtpFrom,
        smtpFromName:
  ...
  async updateContactSettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeString(payload.supportEmail) :
        existing.supportEmail,
        supportLineId:
        payload.supportLineId !== undefined ?
        this.normalizeString(payload.supportLineId) :
        existing.supportLineId
      }
    });
    return this.getPublicSettings();
  }
  async updateBrandingSettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeString(payload.appName) :
        existing.brandingAppName,
        brandingLogoUrl:
        payload.logoUrl !== undefined ?
        this.normalizeString(payload.logoUrl) :
        existing.brandingLogoUrl,
  ...
  async updatePasswordPolicySettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeInteger(
          payload.minLength,
          existing.passwordMinLength ?? DEFAULT_PASSWORD_MIN_LENGTH,
          6,
          64
        ) :
  ...
  async updateRateLimitSettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeInteger(
          payload.authLoginLimit,
          existing.rateAuthLoginLimit ?? DEFAULT_AUTH_LOGIN_RATE_LIMIT,
          1,
          500
        ) :
  ...
  async updateBackupSettings(payload: {
    ...
    await this.prisma.systemSetting.update({
      ...
      data: {
        ...
        this.normalizeString(payload.directory) :
  ...
  async updateInviteSettings(payload: {expiryDays?: number | string | null;}) {
    ...
    this.normalizeInviteExpiryDays(
      payload.expiryDays,
      existing.inviteExpiryDays
    ) :
  ...
  async getEffectiveEmailSettings(): Promise<EffectiveEmailSettings> {
    const settings = await this.getOrCreateSettingsRow();
    const envHost = this.normalizeString(
      this.configService.get<string>('SMTP_HOST')
    );
    const envPort = this.normalizePort(
      this.configService.get<string>('SMTP_PORT'),
      587
    );
    const envSecure =
    this.configService.get<string>('SMTP_SECURE')?.toLowerCase() === 'true';
    const envUser = this.normalizeString(
      this.configService.get<string>('SMTP_USER')
    );
    const envPass = this.normalizeString(
      this.configService.get<string>('SMTP_PASS')
    );
    const envFrom = this.normalizeString(
      this.configService.get<string>('SMTP_FROM')
    );
    const envFromName = this.normalizeString(
    ...
    return {
      ...
      smtpFrom:
      this.normalizeString(settings.smtpFrom) ||
      envFrom ||
      'noreply@formbuilder.com',
      smtpFromName:
      this.normalizeString(settings.smtpFromName) ||
      envFromName ||
      'Form Builder'
    };
  }
