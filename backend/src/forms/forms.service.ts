...
import { Prisma, FormStatus, RoleType } from '@prisma/client';
import { ActivityLogService } from '../activity-log/activity-log.service';
import { EncryptionService } from '../common/encryption.service';
import {
  CreateFormDto,
  CreateLogicConditionDto,
  CreateLogicActionDto } from
'./dto/create-form.dto';
...
FormsService {
  ...
  async create(userId: string, createFormDto: CreateFormDto) {
    const { fields, conditions, logicRules, folderId, ...formData } =
    createFormDto;
    ...
    await this.activityLog.log(form.id, userId, 'CREATED', {
      title: form.title
    });
    return form;
  }
  ...
  updateFormDto: UpdateFormDto)
  {
    ...
    const isCollaborator = originalForm.collaborators?.some(
      (c) => c.id === userId
    );
    const isAdmin =
    userRole === RoleType.SUPER_ADMIN || userRole === RoleType.ADMIN;
    if (!isAdmin && originalForm.createdById !== userId && !isCollaborator) {
      throw new ForbiddenException(
        'You can only edit your own forms or forms shared with you'
      );
    }
    const { fields, conditions, logicRules, folderId, ...formData } =
    updateFormDto;
    ...
    const activityDetails = FormDiffHelper.calculateDiff(
      originalForm as unknown as Record<string, unknown>,
      {
        fields: fields as any,
        logicRules: logicRules as any,
        ...formData
      } as any
    );
    try {
      await this.prisma.$transaction(async (prisma) => {
        ...
        if (logicRules)
        await prisma.logicRule.deleteMany({ where: { formId: id } });
        if (fields || conditions)
        await prisma.fieldCondition.deleteMany({ where: { formId: id } });
        if (fields) {
          const existingFields = await prisma.field.findMany({
            where: { formId: id },
            select: { id: true }
          });
          const { toDelete, toCreate, toUpdate } =
          FieldUpdateHelper.identifyFieldOperations(existingFields, fields);
          ...
          if (toUpdate.length > 0) {
            await Promise.all(
              ...
              prisma.field.update({
                where: { id: field.id },
                data: FieldUpdateHelper.prepareFieldForUpdate(
                  field
          ...
          if (toCreate.length > 0) {
            await prisma.field.createMany({
              ...
              FieldUpdateHelper.prepareFieldForCreate(field, id)
        ...
        if (logicRules) {
          ...
          if (fields) {
            ...
            const currentFields = await prisma.field.findMany({
              where: { formId: id },
              select: { id: true }
            });
          ...
          for (const rule of logicRules) {
            const validConditions = FieldUpdateHelper.filterValidLogicItems(
              rule.conditions || [],
              validFieldIds
            ) as CreateLogicConditionDto[];
            const validActions = FieldUpdateHelper.filterValidLogicItems(
              rule.actions || [],
              validFieldIds
            ) as CreateLogicActionDto[];
  ...
  async remove(id: string, userId: string, userRole: RoleType) {
    ...
    await this.activityLog.log(id, userId, 'DELETED', {
      formId: id,
      title: form.title
    });
  ...
  async clone(id: string, userId: string) {
    ...
    if (!isCreator && !isCollaborator) {
      throw new ForbiddenException(
        'You can only clone your own forms or forms shared with you'
      );
    }
