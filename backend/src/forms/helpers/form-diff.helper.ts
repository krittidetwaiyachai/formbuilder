...
export class FormDiffHelper {
  ...
  private static getSettingsChanges(originalForm: Record<string, unknown>, formData: Record<string, unknown>) {
    ...
    const themeChange = settingsChanges.find((c) => {
      const p = String(c.property || '').toLowerCase().replace(/[^a-z]/g, '');
  ...
  private static calculateFieldChanges(originalFields: Record<string, any>[], newFields: Record<string, any>[], activityDetails: FormDiffDetails) {
  ...
  private static getFieldPropertyChanges(oldField: Record<string, any>, newField: Record<string, any>) {
  ...
  private static compareNestedObject(oldObj: Record<string, any>, newObj: Record<string, any>, prefix: string, changes: Record<string, unknown>[], normalize: (v: unknown) => unknown) {
    ...
    keys.forEach((key) => {
      ...
      if (key === 'items') {
        ...
        const sortedOld = [...oldItems].sort((a: unknown, b: unknown) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
        const sortedNew = [...newItems].sort((a: unknown, b: unknown) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
  ...
  private static calculateLogicChanges(originalLogicRules: Record<string, any>[], newLogicRules: Record<string, any>[], activityDetails: FormDiffDetails) {
    ...
    newLogicRules.forEach((newRule: Record<string, any>) => {
      if (originalRuleIds.has(newRule.id)) {
        ...
        const simplify = (items: Record<string, any>[]) => items.map((item: Record<string, any>) => {
          const clean: Record<string, unknown> = { fieldId: item.fieldId || null };
