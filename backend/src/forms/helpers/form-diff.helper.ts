export interface FormDiffDetails {
  changes: string[];
...
export class FormDiffHelper {
  ...
  {
    const activityDetails: FormDiffDetails = {
      changes: [],
      addedFields: [],
      deletedFields: [],
      updatedFields: []
    ...
    if (settingsChanges.length > 0) {
      ...
      activityDetails.changes = [
      ...activityDetails.changes,
      ...settingsChanges.map((c) => c.property)];
    }
    if (newFormData.fields) {
      this.calculateFieldChanges(
        originalForm.fields as Record<string, any>[] || [],
        newFormData.fields as Record<string, any>[] || [],
        activityDetails
      );
    }
    if (newFormData.logicRules) {
      this.calculateLogicChanges(
        originalForm.logicRules as Record<string, any>[] || [],
        newFormData.logicRules as Record<string, any>[] || [],
        activityDetails
      );
    }
  ...
  formData: Record<string, unknown>)
  {
    ...
    const themeChange = settingsChanges.find((c) => {
      const p = String(c.property || '').
      toLowerCase().
      replace(/[^a-z]/g, '');
      return p.includes('theme');
    });
    if (themeChange) {
      return [themeChange];
    }
  ...
  activityDetails: FormDiffDetails)
  {
    ...
    originalFields.forEach((f: Record<string, any>) => {
      if (!newFieldIds.has(f.id)) {
        activityDetails.deletedFields.push({
          id: f.id,
          type: f.type,
          label: f.label,
          groupId: f.groupId
    ...
    newFields.forEach((newField: Record<string, any>) => {
  ...
  private static getFieldPropertyChanges(
  oldField: Record<string, any>,
  ...
  private static compareNestedObject(
  ...
  normalize: (v: unknown) => unknown)
  {
    ...
    keys.forEach((key) => {
      ...
      if (key === 'items') {
        ...
        const sortedOld = [...oldItems].sort((a: unknown, b: unknown) =>
        JSON.stringify(a).localeCompare(JSON.stringify(b))
        );
        const sortedNew = [...newItems].sort((a: unknown, b: unknown) =>
        JSON.stringify(a).localeCompare(JSON.stringify(b))
        );
  ...
  private static calculateLogicChanges(
  originalLogicRules: Record<string, any>[],
  newLogicRules: Record<string, any>[],
  activityDetails: FormDiffDetails)
  {
    const logicChanges: {
      added: Record<string, unknown>[];
    ...
    originalLogicRules.forEach((r: Record<string, any>) => {
      if (!newRuleIds.has(r.id)) {
        logicChanges.deleted.push({
          id: r.id,
          name: r.name,
          type: r.logicType
        });
    ...
    newLogicRules.forEach((newRule: Record<string, any>) => {
      if (originalRuleIds.has(newRule.id)) {
        ...
        if (oldRule.logicType !== newRule.logicType) {
          ruleChanges.push({
            property: 'logicType',
            before: oldRule.logicType,
            after: newRule.logicType
          });
        }
        const simplify = (items: Record<string, any>[]) =>
    ...
    if (
    logicChanges.added.length > 0 ||
    logicChanges.deleted.length > 0 ||
    logicChanges.updated.length > 0)
    {
