...
import { SystemMaintenanceService } from '../system-settings/system-maintenance.service';
...
AdminService {
  ...
  code: string)
  {
    return crypto.
    createHash('sha256').
    update(`${verificationId}|${normalizedEmail}|${code}`).
    digest('hex');
  }
  async requestLocalUserEmailVerification(rawEmail: string) {
    ...
    await this.systemSettingsService.getRateLimitSettings()).
    verificationCooldownSeconds;
    const latest = await (
    this.prisma as any).
    adminLocalUserEmailVerification.findFirst({
      where: {
        normalizedEmail,
        verifiedAt: null,
    ...
    const resendAvailableAt = new Date(
      Date.now() + Math.max(30, cooldownSeconds || 60) * 1000
    );
    const created = await (
    this.prisma as any).
    adminLocalUserEmailVerification.create({
      data: {
  ...
  async verifyLocalUserEmail(payload: {
    verificationId: string;
    email: string;
    code: string;
  }) {
    ...
    this.prisma as any).
    adminLocalUserEmailVerification.findUnique({
    ...
    const expected = this.hashOtpCode(
      verification.id,
      normalizedEmail,
      payload.code
    );
  ...
  async deleteForm(formId: string, adminUserId?: string) {
    const form = await this.prisma.form.findUnique({
      where: { id: formId },
    ...
    if (form) {
      this.logger.warn(
        `ADMIN_DELETE_FORM | formId=${formId} | title="${form.title}" | deletedBy=${adminUserId || 'unknown'}`
      );
    }
  ...
  async getSystemLogs(params: {
    ...
    const sanitizedLogs = logs.map((log) => ({
      ...log,
      details: this.sanitizeSystemLogDetails(
        log.action,
        log.details as Prisma.JsonValue | null
      )
    }));
    ...
    const removerIds = Array.from(
      new Set(
        ...
        map((log) => {
          ...
          null;
          return typeof details?.removedBy === 'string' ?
          details.removedBy :
          null;
        }).
    ...
    const normalizedLogs = filteredLogs.map((log) => {
      ...
      null;
      const removedBy =
      typeof details?.removedBy === 'string' ? details.removedBy : null;
      if (!removedBy) {
        return log;
      }
      ...
      {};
      if (
      typeof normalizedDetails.removedUserId !== 'string' ||
      normalizedDetails.removedUserId.length === 0)
      {
    ...
    const adjustedTotal = Math.max(
      0,
      total - (sanitizedLogs.length - filteredLogs.length)
    );
    return {
      data: normalizedLogs,
      meta: {
  ...
  async updateSystemInviteSettings(payload: {
  ...
  async runSystemRetentionCleanup() {
    return this.systemMaintenanceService.runRetentionCleanupNow();
  }
  ...
  async runSystemBackupNow() {
    return this.systemMaintenanceService.runBackupNow();
  }
  async getSystemBackupFiles() {
    return this.systemMaintenanceService.getBackupFilesList();
  }
  async deleteSystemBackup(fileName: string) {
    return this.systemMaintenanceService.deleteBackupFile(fileName);
  }
  async getSystemBackupFilePath(fileName: string) {
    return this.systemMaintenanceService.getBackupFilePath(fileName);
  }
  async restoreSystemBackupLatest(confirm?: boolean, fileName?: string) {
    return this.systemMaintenanceService.restoreLatestBackup(confirm, fileName);
  }
  async sendSystemTestEmail(to: string) {
    return this.mailService.sendTestEmail({ to });
  }
  private sanitizeSystemLogDetails(
  action: string,
