...
ResponsePersistenceService {
    ...
    return {
      ...
      quizReview: form.isQuiz ?
      {
        answers: response.answers.map((answer) => ({
          ...
          correctAnswer: (form.quizSettings as QuizSettings | null)?.
          showAnswer ?
        ...
        showAnswer:
        (form.quizSettings as QuizSettings | null)?.showAnswer || false,
        showDetail:
        (form.quizSettings as QuizSettings | null)?.showDetail || false
      } :
      undefined
    };
  }
  ...
  form: Awaited<ReturnType<ResponsePersistenceService['loadPublishedForm']>>)
  {
    ...
    if (settings.endTime) {
      ...
      if (now > endTime) {
        throw new ForbiddenException(
          `This quiz closed on ${endTime.toLocaleString()}`
        );
      }
    }
  }
