...
UnifiedPublicSubmissionService {
  ...
  userAgent?: string)
  {
    const {
      ...
      isNew
    } = await this.publicSessionService.ensureSession(
      formId,
      cookieToken,
      ipAddress,
      userAgent
    );
    const form = await this.formsService.findPublic(
      formId,
      undefined,
      ipAddress,
      userAgent
    );
    const submissionState = await this.getSubmissionStateForSession(
      formId,
      session.id
    );
    return {
  ...
  userAgent?: string)
  {
    ...
    const form =
    await this.responsePersistenceService.loadPublishedForm(formId);
    ...
    await this.prisma.submissionVerificationRequest.findFirst({
      where: {
        formId,
  ...
  verificationRequestId: string)
  {
    ...
    const session = await this.publicSessionService.resolveSession(
      formId,
      cookieToken
    );
    if (!session) {
    ...
    try {
      const submission = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (security.requireEmailVerification) {
            const verificationRequest =
            await tx.submissionVerificationRequest.findFirst({
              where: {
                formId,
                sessionId: session.id,
                canonicalEmailNormalized: canonicalEmail || '',
          ...
          const savedResponse =
          await this.responsePersistenceService.createInTransaction(
            tx,
            form,
            {
              formId,
          ...
          if (
          (form.settings as {allowMultipleSubmissions?: boolean;} | null)?.
  ...
  private async getSubmissionStateForSession(
  formId: string,
  sessionId: string)
  {
    const [session, latestVerificationRequest, form] = await Promise.all([
    ...
    (form?.settings as {allowMultipleSubmissions?: boolean;} | null)?.
    allowMultipleSubmissions === true;
    if (session?.submittedAt && !allowMultiple) {
  ...
  private buildVerificationUrl(token: string): string {
    ...
    return new URL(
      `/api/public/email-verifications/${token}`,
      publicUrl
    ).toString();
  }
