...
import { CanonicalEmailSource } from './form-security-settings.util';
import { normalizeEmail } from './disposable-email.util';
...
export function resolveCanonicalEmail(params: {
  ...
  const { source, topLevelEmail, answers, formFields } = params;
  if (!source) {
  ...
  topLevelEmail && topLevelEmail.trim() ?
  normalizeEmail(topLevelEmail) :
  null;
  if (source.mode === 'top_level') {
    ...
    formFields.some(
      (field) =>
      field.id === answer.fieldId && field.type === FieldType.EMAIL
    )
    ).
    map((answer) => normalizeEmail(answer.value)).
    filter(Boolean);
    const mismatch = answerEmails.some(
      (value) => value !== normalizedTopLevelEmail
    );
    if (mismatch) {
      throw new BadRequestException({
  ...
  const canonicalField = formFields.find(
    (field) => field.id === source.fieldId
  );
  ...
  const fieldAnswer = answers.find(
    (answer) => answer.fieldId === source.fieldId
  );
  ...
  if (
  normalizedTopLevelEmail &&
  normalizedTopLevelEmail !== normalizedFieldEmail)
  {
    throw new BadRequestException({
      code: 'EMAIL_SOURCE_MISMATCH',
      message: 'Email values do not match.'
