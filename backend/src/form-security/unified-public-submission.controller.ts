...
UnifiedPublicSubmissionController {
  constructor(
  private readonly unifiedPublicSubmissionService: UnifiedPublicSubmissionService)
  {}
  @Get('forms/:formId')
  async getForm(
    ...
    @Headers('user-agent')userAgent?: string)
  {
    const cookieToken = this.readSessionCookie(req, formId);
    const result =
    await this.unifiedPublicSubmissionService.requestVerification(
      formId,
      cookieToken,
      body,
      ipAddress,
  ...
  async verifyEmailToken(
    ...
    @Res()res: Response)
  {
    const result = await this.unifiedPublicSubmissionService.verifyEmailToken(
      token,
      ipAddress
    );
    const html =
